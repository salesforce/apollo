...
public class TransactionsTest {
	...
	public void consecutiveCounter() throws Exception {
		...
		try {
			...
			for (int i = ordered.size() - 1; i >= 3; i--) {
				...
				create.transaction(config -> dag
						.tryFinalize(   ordered.stream().map(e -> e.bytes()).collect(Collectors.toList()),
										DSL.using(config)));
			}

			assertEquals(   parameters.beta2,
							create.select(CONFLICTSET.COUNTER).from(CONFLICTSET).join(UNFINALIZED)
									.on(UNFINALIZED.HASH.eq(CONFLICTSET.PREFERRED))
									.and(UNFINALIZED.HASH.eq(firstCommit.bytes())).fetchOne().value1().intValue());
			assertEquals(   parameters.beta2,
							create.select(CONFLICTSET.COUNTER).from(CONFLICTSET).join(UNFINALIZED)
									.on(UNFINALIZED.HASH.eq(CONFLICTSET.PREFERRED))
									.and(UNFINALIZED.HASH.eq(secondCommit.bytes())).fetchOne().value1().intValue());
			...
			create.transactionResult(config -> dag
					.tryFinalize(ordered.stream().map(e -> e.bytes()).collect(Collectors.toList()), DSL.using(config)));
	...
	public void earlyCommit() throws Exception {
		...
		assertEquals(parameters.beta1, create.select(UNFINALIZED.CONFIDENCE).from(UNFINALIZED)
				.where(UNFINALIZED.HASH.eq(firstCommit.bytes())).fetchOne().value1().intValue());

		assertEquals(parameters.beta1, create.select(UNFINALIZED.CONFIDENCE).from(UNFINALIZED)
				.where(UNFINALIZED.HASH.eq(secondCommit.bytes())).fetchOne().value1().intValue());
	...
	public void finalizedSet() {
		...
		for (int i = ordered.size() - 1; i >= ordered.size() - 11; i--) {
			dag.prefer(ordered.get(i).toHash(), create);
			finalized = create.transactionResult(config -> dag
					.tryFinalize(ordered.stream().map(e -> e.bytes()).collect(Collectors.toList()), DSL.using(config)));
	...
	public void frontier() throws Exception {
		...
		TreeSet<HashKey> frontier = dag.getNeglectedFrontier(create).map(e -> new HashKey(e))
	...
	public void isStronglyPreferred() throws Exception {
		...
		assertFalse("Not exist returned true: ",
		...
		assertArrayEquals("Aggregate failed: ", new Boolean[] { false, false }, create
				.transactionResult(config -> dag.isStronglyPreferred(Arrays.asList(zero, one), DSL.using(config)))
				.toArray(new Boolean[2]));

		// All are strongly preferred
		for (int i = 0; i < ordered.size(); i++) {
			int it = i;
			assertTrue(String.format("node %s is not strongly preferred", i), create
					.transactionResult(config -> dag.isStronglyPreferred(ordered.get(it).toHash(), DSL.using(config))));
		}
		...
		assertFalse(String.format("node 4 is strongly preferred: " + ordered.get(4)), create
				.transactionResult(config -> dag.isStronglyPreferred(ordered.get(4).toHash(), DSL.using(config))));
		...
		assertFalse(String.format("node 5 is strongly preferred"), create
				.transactionResult(config -> dag.isStronglyPreferred(ordered.get(5).toHash(), DSL.using(config))));

		Boolean[] expected = new Boolean[] { true, true, true, true, false, false };
		assertArrayEquals(  "Aggregate failed: ", expected,
							create.transactionResult(config -> dag.isStronglyPreferred(
																						ordered.stream()
																								.map(e -> e.toHash())
																								.collect(Collectors
																										.toList()),
	...
	public void parentSelectionWithPreferred() throws Exception {
		...
		Set<HashKey> frontier = dag.frontierSample(create).map(r -> new HashKey(r))
				.collect(Collectors.toCollection(TreeSet::new));
		assertEquals(5, frontier.size());
...
//		dumpClosure(ordered, create);
		...
		assertEquals(Integer.valueOf(1), create.select(UNFINALIZED.CHIT).from(UNFINALIZED)
				.where(UNFINALIZED.HASH.eq(ordered.get(3).bytes())).fetchOne().value1());
		assertEquals(Integer.valueOf(1), create.select(UNFINALIZED.CONFIDENCE).from(UNFINALIZED)
				.where(UNFINALIZED.HASH.eq(ordered.get(0).bytes())).fetchOne().value1());
		assertEquals(Integer.valueOf(1), create.select(UNFINALIZED.CONFIDENCE).from(UNFINALIZED)
				.where(UNFINALIZED.HASH.eq(ordered.get(2).bytes())).fetchOne().value1());
		assertEquals(Integer.valueOf(0), create.select(UNFINALIZED.CONFIDENCE).from(UNFINALIZED)
				.where(UNFINALIZED.HASH.eq(ordered.get(4).bytes())).fetchOne().value1());
		...
		assertFalse(String.format("node 4 is strongly preferred"), create
				.transactionResult(config -> dag.isStronglyPreferred(ordered.get(4).toHash(), DSL.using(config))));
		...
		assertEquals(Integer.valueOf(2), create.select(UNFINALIZED.CONFIDENCE).from(UNFINALIZED)
				.where(UNFINALIZED.HASH.eq(ordered.get(0).bytes())).fetchOne().value1());
		assertEquals(Integer.valueOf(2), create.select(UNFINALIZED.CONFIDENCE).from(UNFINALIZED)
				.where(UNFINALIZED.HASH.eq(ordered.get(2).bytes())).fetchOne().value1());
		assertEquals(Integer.valueOf(1), create.select(UNFINALIZED.CONFIDENCE).from(UNFINALIZED)
		...
		assertTrue(String.format("node 3 is not strongly preferred " + ordered.get(3)), create
				.transactionResult(config -> dag.isStronglyPreferred(ordered.get(3).toHash(), DSL.using(config))));
		assertFalse(String.format("node 4 is strongly preferred"), create
				.transactionResult(config -> dag.isStronglyPreferred(ordered.get(4).toHash(), DSL.using(config))));
		...
		assertEquals(Integer.valueOf(3), create.select(UNFINALIZED.CONFIDENCE).from(UNFINALIZED)
				.where(UNFINALIZED.HASH.eq(ordered.get(0).bytes())).fetchOne().value1());
		assertEquals(Integer.valueOf(3), create.select(UNFINALIZED.CONFIDENCE).from(UNFINALIZED)
				.where(UNFINALIZED.HASH.eq(ordered.get(2).bytes())).fetchOne().value1());
		assertEquals(Integer.valueOf(1), create.select(UNFINALIZED.CONFIDENCE).from(UNFINALIZED)
		...
		assertFalse(String.format("node 3 is strongly preferred " + ordered.get(3)), create
				.transactionResult(config -> dag.isStronglyPreferred(ordered.get(3).toHash(), DSL.using(config))));
		assertTrue(String.format("node 4 is not strongly preferred"), create
				.transactionResult(config -> dag.isStronglyPreferred(ordered.get(4).toHash(), DSL.using(config))));
	}
