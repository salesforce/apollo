 ...
 *
 */
public enum ReconOld implements Transitions {
    CHANGE_PRINCIPAL, GATHER {
        ...
        @Override
        public Transitions validateView(Validate validate) {
    ...
    SYNCHRONIZE {
        @Exit
        public void cancelTimer() {
            context().cancelTimer(Driven.SYNCHRONIZE);
        }
        ...
        @Override
        public Transitions sync(Sync sync, Digest from) {
            context().sync(sync, from);
            return null;
        }
