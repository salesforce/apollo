...
public class HexBloom {
    ...
    /**
     * Construct a HexBloom from the supplied parameters, using default hash
     * functions and fpr
     *
     ...
     */
    public static HexBloom construct(int currentCount, Stream<Digest> currentMembership, Digest initialCrown,
                                     int count) {
        return construct(currentCount, currentMembership, Collections.emptyList(), hashes(count), initialCrown,
                         DEFAULT_FPR);
    }
     ...
     * @param currentMembership - Stream of digests that correspond to the supplied
     *                          crowns
     ...
     * @param initialCrown      - the initial value of the crowns
     * @param fpr               - the false positive rate for the membership bloom
     *                          filter
     * @return the HexBloom built according to spec
     */
    public static HexBloom construct(int currentCount, Stream<Digest> currentMembership, List<Digest> added,
    ...
    public boolean validateCrown(List<Digest> wrapped, List<Function<Digest, Digest>> hashes) {
        if (hashes.size() != crowns.length || wrapped.size() != crowns.length) {
            throw new IllegalArgumentException("Size of supplied hash functions: " + hashes.size()
            + " must equal the # of crowns: " + crowns.length + " or wrapped: " + wrapped.size());
        }
    ...
    /**
     * Answer the wrapped form of the receiver using the default wrapping hash
     * transforms
     */
    ...
    /**
     * Answer the wrapped form of the receiver using the supplied wrapping hash
     * transforms
     * 
