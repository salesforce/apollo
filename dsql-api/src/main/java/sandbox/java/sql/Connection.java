 ...
 * the <code>getMetaData</code> method.
 *
 * <P>
 * <B>Note:</B> When configuring a <code>Connection</code>, JDBC applications
 ...
 * Applications should not invoke SQL commands directly to change the
 * connection's configuration when there is a JDBC method available. By default
 * a <code>Connection</code> object is in auto-commit mode, which means that it
 * automatically commits changes after executing each statement. If auto-commit
 * mode has been disabled, the method <code>commit</code> must be called
 * explicitly in order to commit changes; otherwise, database changes will not
 * be saved.
 * <P>
 * A new <code>Connection</code> object created using the JDBC 2.1 core API has
 * an initially empty type map associated with it. A user may enter a custom
 * mapping for a UDT in this type map. When a UDT is retrieved from a data
 * source with the method <code>ResultSet.getObject</code>, the
 * <code>getObject</code> method will check the connection's type map to see if
 * there is an entry for that UDT. If so, the <code>getObject</code> method will
 * map the UDT to the class indicated. If there is no entry, the UDT will be
 * mapped using the standard mapping.
 * <p>
 * A user may create a new type map, which is a <code>java.util.Map</code>
 * object, make an entry in it, and pass it to the <code>sandbox.java.sql</code>
 * methods that can perform custom mapping. In this case, the method will use
 * the given type map instead of the one associated with the connection.
 * <p>
 * For example, the following code fragment specifies that the SQL type
 * <code>ATHLETES</code> will be mapped to the class <code>Athletes</code> in
 * the Java programming language. The code fragment retrieves the type map for
 * the <code>Connection
 * </code> object <code>con</code>, inserts the entry into it, and then sets the
...
public interface Connection extends Wrapper, AutoCloseable {
     ...
     * procedures. The <code>CallableStatement</code> object provides methods for
     * setting up its IN and OUT parameters, and methods for executing the call to a
     * stored procedure.
     *
     * <P>
     * <B>Note:</B> This method is optimized for handling stored procedure call
     * statements. Some drivers may send the call statement to the database when the
     * method <code>prepareCall</code> is done; others may wait until the
     * <code>CallableStatement</code> object is executed. This has no direct effect
     * on users; however, it does affect which method throws certain SQLExceptions.
     * <P>
     * Result sets created using the returned <code>CallableStatement</code> object
     * will by default be type <code>TYPE_FORWARD_ONLY</code> and have a concurrency
     * level of <code>CONCUR_READ_ONLY</code>. The holdability of the created result
     * sets can be determined by calling {@link #getHoldability}.
     *
     ...
     * <code>commit</code> or the method <code>rollback</code>. By default, new
     ...
     * multiple results, the statement is complete when all of the associated result
     * sets have been closed, and all update counts and output parameters have been
     * retrieved.
     * </ul>
     * <P>
     * <B>NOTE:</B> If this method is called during a transaction and the
     * auto-commit mode is changed, the transaction is committed. If
     * <code>setAutoCommit</code> is called and the auto-commit mode is not changed,
     * the call is a no-op.
     *
     * @param autoCommit <code>true</code> to enable auto-commit mode;
     *                   <code>false</code> to disable it
     * @exception SQLException if a database access error occurs,
     *                         setAutoCommit(true) is called while participating in
     *                         a distributed transaction, or this method is called
     ...
     *                         transaction, if this method is called on a closed
     ...
     * connection is closed if the method <code>close</code> has been called on it
     * or if certain fatal errors have occurred. This method is guaranteed to return
     ...
     * database is valid or invalid. A typical client can determine that a
     * connection is invalid by catching any exceptions that might be thrown when an
    ...
    /**
     * Retrieves a <code>DatabaseMetaData</code> object that contains metadata about
     * the database to which this <code>Connection</code> object represents a
     ...
     *                 disables it
     * @exception SQLException if a database access error occurs, this method is
     *                         called on a closed connection or this method is
     ...
     * committed (a "dirty read"). If any of the changes are rolled back, the second
     ...
     * prevented; phantom reads can occur. This level prohibits a transaction from
     * reading a row with uncommitted changes in it, and it also prohibits the
     * situation where one transaction reads a row, a second transaction alters the
     * row, and the first transaction rereads the row, getting different values the
     ...
     * <code>Connection</code> object to the one given. The constants defined in the
     * interface <code>Connection</code> are the possible transaction isolation
     * levels.
     * <P>
     ...
     * @exception SQLException if a database access error occurs, this method is
     *                         called on a closed connection or the given parameter
    ...
    /**
     * Retrieves the first warning reported by calls on this <code>Connection</code>
    ...
    /**
     * Clears all warnings reported for this <code>Connection</code> object. After a
     ...
     * method is the same as the <code>createStatement</code> method above, but it
     ...
     *                                            connection or the given parameters
     *                                            are not <code>ResultSet</code>
     *                                            constants indicating type and
     *                                            concurrency
     * @exception SQLFeatureNotSupportedException if the JDBC driver does not
     *                                            support this method or this method
     *                                            is not supported for the specified
     *                                            result set type and result set
     *                                            concurrency.
     * @since 1.2
     */
     ...
     * <code>ResultSet</code> objects with the given type and concurrency. This
     * method is the same as the <code>prepareStatement</code> method above, but it
     * allows the default result set type and concurrency to be overridden. The
     * holdability of the created result sets can be determined by calling
     * {@link #getHoldability}.
     *
     * @param sql                  a <code>String</code> object that is the SQL
     *                             statement to be sent to the database; may contain
     ...
     * <code>Connection</code> object. Unless the application has added an entry,
     ...
     * driver may create an internal copy of the <code>Map</code> object passed to
     ...
     * <p>
     * You must set the values for the <code>TypeMap</code> prior to callng
     ...
     * @exception SQLException                    if a database access error occurs,
     *                                            this method is called on a closed
     *                                            connection or the given parameter
     *                                            is not a
     ...
     * default holdability of <code>ResultSet</code> objects can be determined by
     ...
     * @exception SQLFeatureNotSupportedException if the given holdability is not
     *                                            supported
     * @see #getHoldability
     ...
     *                                            participating in a distributed
     *                                            transaction, this method is called
     *                                            on a closed connection, the
     *                                            <code>Savepoint</code> object is
     *                                            no longer valid, or this
     *                                            <code>Connection</code> object is
     *                                            currently in auto-commit mode
     * @exception SQLFeatureNotSupportedException if the JDBC driver does not
     *                                            support this method
     ...
     * <code>Savepoint</code> objects from the current transaction. Any reference to
     * the savepoint after it have been removed will cause an
     * <code>SQLException</code> to be thrown.
     *
     * @param savepoint the <code>Savepoint</code> object to be removed
     * @exception SQLException                    if a database access error occurs,
     *                                            this method is called on a closed
     *                                            connection or the given
     *                                            <code>Savepoint</code> object is
     *                                            not a valid savepoint in the
     *                                            current transaction
     * @exception SQLFeatureNotSupportedException if the JDBC driver does not
     *                                            support this method
     * @since 1.4
     ...
     *                                            support this method or this method
     *                                            is not supported for the specified
     *                                            result set type, result set
     *                                            holdability and result set
     *                                            concurrency.
     * @see ResultSet
     * @since 1.4
     */
     ...
     * but it allows the default result set type, concurrency, and holdability to be
     * overridden.
     *
     * @param sql                  a <code>String</code> object that is the SQL
     *                             statement to be sent to the database; may contain
     ...
     *         pre-compiled SQL statement, that will generate <code>ResultSet</code>
     ...
     * This parameter is ignored if the SQL statement is not an <code>INSERT</code>
     ...
     * case, the statement may not be sent to the database until the
     * <code>PreparedStatement</code> object is executed. This has no direct effect
     * on users; however, it does affect which methods throw certain SQLExceptions.
     * <P>
     * Result sets created using the returned <code>PreparedStatement</code> object
     * will by default be type <code>TYPE_FORWARD_ONLY</code> and have a concurrency
     * level of <code>CONCUR_READ_ONLY</code>. The holdability of the created result
     ...
     * array if the SQL statement is not an <code>INSERT</code> statement, or an SQL
     * statement able to return auto-generated keys (the list of such statements is
     * vendor-specific).
     * <p>
     * An SQL statement with or without IN parameters can be pre-compiled and stored
     * in a <code>PreparedStatement</code> object. This object can then be used to
     * efficiently execute this statement multiple times.
     * <P>
     * <B>Note:</B> This method is optimized for handling parametric SQL statements
     ...
     * keys that should be returned. The driver will ignore the array if the SQL
     * statement is not an <code>INSERT</code> statement, or an SQL statement able
     ...
     * @since 1.6
     *
     * @see sandbox.java.sql.DatabaseMetaData#getClientInfoProperties
     */
     ...
     *                                server or this method is called on a closed
     *                                connection
     *
     * @since 1.6
     */
     ...
     *
     * @see sandbox.java.sql.Connection#setClientInfo(String, String)
     *      setClientInfo(String, String)
     * @since 1.6
     *
     * @throws SQLClientInfoException if the database server returns an error while
     *                                setting the clientInfo values on the database
     *                                server or this method is called on a closed
     *                                connection
     *
     */
     ...
     *
     * @throws SQLException if the database server returns an error when fetching
     *                      the client info value from the database or this method
     ...
     *
     * @throws SQLException if the database server returns an error when fetching
     *                      the client info values from the database or this method
     *                      is called on a closed connection
     *
     * @since 1.6
     */
     ...
     * object that maps to a primitive data type, then it is implementation-defined
     * whether the <code>Array</code> object is an array of that primitive data type
     * or an array of <code>Object</code>.
     * <p>
     * <b>Note: </b>The JDBC driver is responsible for mapping the elements
     * <code>Object</code> array to the default JDBC SQL type defined in
     * sandbox.java.sql.Types for the given class of <code>Object</code>. The
     * default mapping is specified in Appendix B of the JDBC specification. If the
     * resulting JDBC type is not the appropriate type for the given typeName then
     * it is implementation defined whether an <code>SQLException</code> is thrown
     * or the driver supports the resulting conversion.
     *
     ...
     *                 returned by <code>Array.getBaseTypeName</code>
     * @param elements the elements that populate the returned object
     * @return an Array object whose elements map to the specified SQL type
     * @throws SQLException                    if a database error occurs, the JDBC
     *                                         type is not appropriate for the
     *                                         typeName and the conversion is not
     *                                         supported, the typeName is null or
     *                                         this method is called on a closed
     *                                         connection
     ...
     *                   name of a user-defined type that has been defined for this
     ...
     * @throws SQLException                    if a database error occurs, the
     *                                         typeName is null or this method is
     *                                         called on a closed connection
     * @throws SQLFeatureNotSupportedException if the JDBC driver does not support
     *                                         this data type
     * @since 1.6
     ...
     * prepare operation takes place immediately when the {@code Connection} method
     * {@code prepareStatement} or {@code prepareCall} is invoked. For maximum
     * portability, {@code setSchema} should be called before a {@code Statement} is
     * created or prepared.
     *
     ...
     * either progress to completion or throw an <code>SQLException</code>.
     * </ul>
     * <p>
     ...
     * by the connection can take an extended period of time. When the
     ...
     * <p>
     * This method checks to see that there is an <code>SQLPermission</code> object
     * before allowing the method to proceed. If a <code>SecurityManager</code>
     * exists and its <code>checkPermission</code> method denies calling
     * <code>abort</code>, this method throws a
     * <code>java.lang.SecurityException</code>.
     * 
     * @param executor The <code>Executor</code> implementation which will be used
     *                 by <code>abort</code>.
     * @throws sandbox.java.sql.SQLException if a database access error occurs or
     *                                       the {@code executor} is {@code null},
     * @throws java.lang.SecurityException   if a security manager exists and its
     *                                       <code>checkPermission</code> method
     *                                       denies calling <code>abort</code>
    ...
    /**
     *
     * Sets the maximum period a <code>Connection</code> or objects created from the
     * <code>Connection</code> will wait for the database to reply to any one
     * request. If any request remains unanswered, the waiting method will return
     * with a <code>SQLException</code>, and the <code>Connection</code> or objects
     * created from the <code>Connection</code> will be marked as closed. Any
     * subsequent use of the objects, with the exception of the <code>close</code>,
     * <code>isClosed</code> or <code>Connection.isValid</code> methods, will result
     * in a <code>SQLException</code>.
     * <p>
     * <b>Note</b>: This method is intended to address a rare but serious condition
     * where network partitions can cause threads issuing JDBC calls to hang
     * uninterruptedly in socket reads, until the OS TCP-TIMEOUT (typically 10
     * minutes). This method is related to the {@link #abort abort() } method which
     * provides an administrator thread a means to free any such threads in cases
     * where the JDBC connection is accessible to the administrator thread. The
     * <code>setNetworkTimeout</code> method will cover cases where there is no
     * administrator thread, or it has no access to the connection. This method is
     * severe in it's effects, and should be given a high enough value so it is
     * never triggered before any more normal timeouts, such as transaction
     * timeouts.
     * <p>
     * JDBC driver implementations may also choose to support the
     * {@code setNetworkTimeout} method to impose a limit on database response time,
     * in environments where no network is present.
     * <p>
     * Drivers may internally implement some or all of their API calls with multiple
     * internal driver-database transmissions, and it is left to the driver
     * implementation to determine whether the limit will be applied always to the
     * response to the API call, or to any single request made during the API call.
     * <p>
     *
     * This method can be invoked more than once, such as to set a limit for an area
     * of JDBC code, and to reset to the default on exit from this area. Invocation
     * of this method has no impact on already outstanding requests.
     * <p>
     ...
     * expires before the network timeout then the statement execution will be
     * canceled. If the network is still active the result will be that both the
     * statement and connection are still usable. However if the network timeout
     * expires before the query timeout or if the statement timeout fails due to
     * network problems, the connection will be marked as closed, any resources held
     * by the connection will be released and both the connection and statement will
     * be unusable.
     * <p>
     * When the driver determines that the {@code setNetworkTimeout} timeout value
     * has expired, the JDBC driver marks the connection closed and releases any
     ...
     *                     used by <code>setNetworkTimeout</code>.
     ...
     * @throws sandbox.java.sql.SQLException if a database access error occurs, this
     *                                       method is called on a closed
     *                                       connection, the {@code executor} is
     *                                       {@code null}, or the value specified
     *                                       for <code>seconds</code> is less than
     *                                       0.
     * @throws java.lang.SecurityException   if a security manager exists and its
     *                                       <code>checkPermission</code> method
     *                                       denies calling
     ...
     * request to complete. If the limit is exceeded, a <code>SQLException</code> is
     * thrown.
     *
     * @return the current timeout limit in milliseconds; zero means there is no
     *         limit
     * @throws SQLException if a database access error occurs or this method is
     *                      called on a closed <code>Connection</code>
     * @exception SQLFeatureNotSupportedException if the JDBC driver does not
     *                                            support this method
     ...
     * on this connection. Each request is independent of all other requests with
     * regard to state local to the connection either on the client or the server.
     * Work done between {@code beginRequest}, {@code endRequest} pairs does not
     * depend on any other work done on the connection either as part of another
     * request or outside of any request. A request may include multiple
     * transactions. There may be dependencies on committed database state as that
     * is not local to the connection.
     * <p>
     * Local state is defined as any state associated with a Connection that is
     * local to the current Connection either in the client or the database that is
     * not transparently reproducible.
     * <p>
     * Calls to {@code beginRequest} and {@code endRequest} are not nested. Multiple
     * calls to {@code beginRequest} without an intervening call to
     * {@code endRequest} is not an error. The first {@code beginRequest} call marks
     * the start of the request and subsequent calls are treated as a no-op
     * <p>
     * Use of {@code beginRequest} and {@code endRequest} is optional, vendor
     * specific and should largely be transparent. In particular implementations may
     * detect conditions that indicate dependence on other work such as an open
     * transaction. It is recommended though not required that implementations throw
     * a {@code SQLException} if there is an active transaction and
     * {@code beginRequest} is called. Using these methods may improve performance
     * or provide other benefits. Consult your vendors documentation for additional
     * information.
     * <p>
     * It is recommended to enclose each unit of work in {@code beginRequest},
     * {@code endRequest} pairs such that there is no open transaction at the
     * beginning or end of the request and no dependency on local state that crosses
     * request boundaries. Committed database state is not local.
     *
     * @implSpec The default implementation is a no-op.
     *
     * @apiNote This method is to be used by Connection pooling managers.
     *          <p>
     *          The pooling manager should call {@code beginRequest} on the
     *          underlying connection prior to returning a connection to the caller.
     *          <p>
     ...
     *          <li>Returns a logical connection handle when {@code getConnection}
     *          is called by the application</li>
     *          <li>The logical {@code Connection} is closed by calling
     *          {@code Connection.close} prior to returning the
     *          {@code PooledConnection} to the cache.</li>
     *          </ul>
    ...
    /**
     * Hints to the driver that a request, an independent unit of work, has
     * completed. Calls to {@code beginRequest} and {@code endRequest} are not
     * nested. Multiple calls to {@code endRequest} without an intervening call to
     * {@code beginRequest} is not an error. The first {@code endRequest} call marks
     * the request completed and subsequent calls are treated as a no-op. If
     * {@code endRequest} is called without an initial call to {@code beginRequest}
     * is a no-op.
     * <p>
     * The exact behavior of this method is vendor specific. In particular
     * implementations may detect conditions that indicate dependence on other work
     * such as an open transaction. It is recommended though not required that
     * implementations throw a {@code SQLException} if there is an active
     ...
     *          <p>
     *          The pooling manager should call {@code endRequest} on the underlying
     *          connection when the applications returns the connection back to the
     *          connection pool.
     *          <p>
     ...
     * value may be specified for the sharding Key. The validity of a {@code null}
     * sharding key is vendor-specific. Consult your vendor&#39;s documentation for
     ...
     *          key is valid before {@code setShardingKeyIfValid} returns false.
     * @param shardingKey      the sharding key to be validated against this
     *                         connection. The sharding key may be {@code null}
     * @param superShardingKey the super sharding key to be validated against this
     *                         connection. The super sharding key may be
     *                         {@code null}.
     * @param timeout          time in seconds before which the validation process
     *                         is expected to be completed, otherwise the validation
     *                         process is aborted. A value of 0 indicates the
     *                         validation process will not time out.
     * @return true if the connection is valid and the sharding keys are valid and
     *         set on this connection; false if the sharding keys are not valid or
     *         the timeout period expires before the operation completes.
     * @throws SQLException                    if an error occurs while performing
     *                                         this validation; a
     *                                         {@code superSharedingKey} is
     *                                         specified without a
     *                                         {@code shardingKey}; this method is
     *                                         called on a closed
     *                                         {@code connection}; or the
     *                                         {@code timeout} value is negative.
    ...
    default boolean setShardingKeyIfValid(ShardingKey shardingKey, ShardingKey superShardingKey,
        ...
        throw new SQLFeatureNotSupportedException(String.toDJVM("setShardingKeyIfValid not implemented"));
    }

    /**
     * Sets and validates the sharding key for this connection. A {@code null} value
     * may be specified for the sharding Key. The validity of a {@code null}
     * sharding key is vendor-specific. Consult your vendor&#39;s documentation for
     * additional information.
     * 
     * @implSpec The default implementation will throw a
     ...
     *                    expected to be completed,else the validation process is
     *                    aborted. A value of 0 indicates the validation process
     *                    will not time out.
     * @return true if the connection is valid and the sharding key is valid to be
     *         set on this connection; false if the sharding key is not valid or the
     *         timeout period expires before the operation completes.
     * @throws SQLException                    if there is an error while performing
     *                                         this validation; this method is
     *                                         called on a closed
     *                                         {@code connection}; or the
     *                                         {@code timeout} value is negative.
     * @throws SQLFeatureNotSupportedException if the driver does not support
     ...
     *          round trip to the database to validate that the sharding keys are
     *          valid for the {@code Connection}.
     * @param shardingKey      the sharding key to set on this connection. The
     *                         sharding key may be {@code null}
     * @param superShardingKey the super sharding key to set on this connection. The
     *                         super sharding key may be {@code null}
     * @throws SQLException                    if an error occurs setting the
     *                                         sharding keys; this method is called
     *                                         on a closed {@code connection}; or a
     *                                         {@code superSharedingKey} is
     *                                         specified without a
     *                                         {@code shardingKey}
     * @throws SQLFeatureNotSupportedException if the driver does not support
    ...
    default void setShardingKey(ShardingKey shardingKey, ShardingKey superShardingKey) throws SQLException {
        throw new SQLFeatureNotSupportedException(String.toDJVM("setShardingKey not implemented"));
    }
     ...
     *          round trip to the database to validate that the sharding key is
     *          valid for the {@code Connection}.
     * @param shardingKey the sharding key to set on this connection. The sharding
     *                    key may be {@code null}
     * @throws SQLException                    if an error occurs setting the
     *                                         sharding key; or this method is
     *                                         called on a closed {@code connection}
     * @throws SQLFeatureNotSupportedException if the driver does not support
     *                                         sharding
     * @since 9
     * @see ShardingKey
