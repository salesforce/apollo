 ...
 * driver based on JDBC technology ("JDBC driver") that is used with it.
 ...
 * deal with the underlying DBMS. This is especially true for applications that
 * are intended to be used with more than one DBMS. For example, a tool might
 * use the method <code>getTypeInfo</code> to find out what data types can be
 * used in a <code>CREATE TABLE</code> statement. Or a user might call the
 * method <code>supportsCorrelatedSubqueries</code> to see if it is possible to
 * use a correlated subquery or <code>supportsBatchUpdates</code> to see if it
 * is possible to use batch updates.
 * <P>
 * Some <code>DatabaseMetaData</code> methods return lists of information in the
 * form of <code>ResultSet</code> objects. Regular <code>ResultSet</code>
 ...
 * form of metadata is not available, an empty <code>ResultSet</code> will be
 * returned. Additional columns beyond the columns defined to be returned by the
 * <code>ResultSet</code> object for a given method can be defined by the JDBC
 * driver vendor and must be accessed by their <B>column label</B>.
 * <P>
 * Some <code>DatabaseMetaData</code> methods take arguments that are String
 * patterns. These arguments all have names such as fooPattern. Within a pattern
 * String, "%" means match any substring of 0 or more characters, and "_" means
...
public interface DatabaseMetaData extends Wrapper {
     ...
     * <code>CONVERT</code> for the conversion of one JDBC type to another. The JDBC
     * types are the generic SQL data types defined in
     ...
     * and <i>toType</i>. The JDBC types are the generic SQL data types defined in
     * <code>sandbox.java.sql.Types</code>.
     *
     * @param fromType the type to convert from; one of the type codes from the
     *                 class <code>sandbox.java.sql.Types</code>
     ...
     * values are defined in <code>sandbox.java.sql.Connection</code>.
     *
     ...
     *              <code>sandbox.java.sql.Connection</code>
     * @return <code>true</code> if so; <code>false</code> otherwise
     * @exception SQLException if a database access error occurs
     * @see Connection
     */
     ...
     * are returned. They are ordered by <code>PROCEDURE_CAT</code>,
     ...
     * are returned. They are ordered by PROCEDURE_CAT, PROCEDURE_SCHEM,
     * PROCEDURE_NAME and SPECIFIC_NAME. Within this, the return value, if any, is
     ...
     * </UL>
     * <LI><B>DATA_TYPE</B> int {@code =>} SQL type from sandbox.java.sql.Types
     * <LI><B>TYPE_NAME</B> String {@code =>} SQL type name, for a UDT type the type
     ...
     * default value
     * <LI>TRUNCATE (not enclosed in quotes) - if the specified default value cannot
     ...
     * <LI><B>CHAR_OCTET_LENGTH</B> int {@code =>} the maximum length of binary and
     ...
     * column. For numeric data, this is the maximum precision. For character data,
     * this is the length in characters. For datetime datatypes, this is the length
     ...
     * <P>
     * A possible value for the column <code>NULLABLE</code> in the
     * <code>ResultSet</code> object returned by the method
     * <code>getProcedureColumns</code>.
     */
     ...
     *                         catalog; <code>null</code> means that the catalog
     ...
     *                         without a schema; <code>null</code> means that the
     ...
     *                         table types returned from {@link #getTableTypes},to
     ...
     * or user-generated Ref type, SQL type from sandbox.java.sql.Types
     ...
     * <P>
     * A possible value for the column <code>NULLABLE</code> in the
     * <code>ResultSet</code> returned by the method <code>getColumns</code>.
     */
    int columnNoNulls = 0;
     ...
     * <LI><B>TABLE_NAME</B> String {@code =>} table name
     * <LI><B>COLUMN_NAME</B> String {@code =>} column name
     * <LI><B>GRANTOR</B> String {@code =>} grantor of access (may be
     * <code>null</code>)
     * <LI><B>GRANTEE</B> String {@code =>} grantee of access
     * <LI><B>PRIVILEGE</B> String {@code =>} name of access (SELECT, INSERT,
     * UPDATE, REFERENCES, ...)
     * <LI><B>IS_GRANTABLE</B> String {@code =>} "YES" if grantee is permitted to
     * grant to others; "NO" if not; <code>null</code> if unknown
     * </OL>
     *
     * @param catalog           a catalog name; must match the catalog name as it is
     *                          stored in the database; "" retrieves those without a
     ...
     * </UL>
     * <LI><B>COLUMN_NAME</B> String {@code =>} column name
     * <LI><B>DATA_TYPE</B> int {@code =>} SQL data type from sandbox.java.sql.Types
     * <LI><B>TYPE_NAME</B> String {@code =>} Data source dependent type name, for a
     * UDT the type name is fully qualified
     * <LI><B>COLUMN_SIZE</B> int {@code =>} precision
     * <LI><B>BUFFER_LENGTH</B> int {@code =>} not used
     * <LI><B>DECIMAL_DIGITS</B> short {@code =>} scale - Null is returned for data
     * types where DECIMAL_DIGITS is not applicable.
     * <LI><B>PSEUDO_COLUMN</B> short {@code =>} is this a pseudo column like an
     * Oracle ROWID
     * <UL>
     ...
     * lasting only while the row is being used.
     * <P>
     * A possible value for the column <code>SCOPE</code> in the
     * <code>ResultSet</code> object returned by the method
     * <code>getBestRowIdentifier</code>.
     */
     ...
     * <P>
     * A possible value for the column <code>PSEUDO_COLUMN</code> in the
     * <code>ResultSet</code> object returned by the method
     * <code>getBestRowIdentifier</code>.
     */
     ...
     * <LI><B>COLUMN_NAME</B> String {@code =>} column name
     * <LI><B>DATA_TYPE</B> int {@code =>} SQL data type from
     * <code>sandbox.java.sql.Types</code>
     ...
     * <LI><B>DECIMAL_DIGITS</B> short {@code =>} scale - Null is returned for data
     * types where DECIMAL_DIGITS is not applicable.
     * <LI><B>PSEUDO_COLUMN</B> short {@code =>} whether this is pseudo column like
     * an Oracle ROWID
     * <UL>
     ...
     * value of 1 represents the first column of the primary key, a value of 2 would
     ...
     * Each primary key column description has the following columns:
     * <OL>
     * <LI><B>PKTABLE_CAT</B> String {@code =>} primary key table catalog being
     * imported (may be <code>null</code>)
     * <LI><B>PKTABLE_SCHEM</B> String {@code =>} primary key table schema being
     * imported (may be <code>null</code>)
     * <LI><B>PKTABLE_NAME</B> String {@code =>} primary key table name being
     * imported
     * <LI><B>PKCOLUMN_NAME</B> String {@code =>} primary key column name being
     * imported
     * <LI><B>FKTABLE_CAT</B> String {@code =>} foreign key table catalog (may be
     * <code>null</code>)
     * <LI><B>FKTABLE_SCHEM</B> String {@code =>} foreign key table schema (may be
     * <code>null</code>)
     * <LI><B>FKTABLE_NAME</B> String {@code =>} foreign key table name
     * <LI><B>FKCOLUMN_NAME</B> String {@code =>} foreign key column name
     * <LI><B>KEY_SEQ</B> short {@code =>} sequence number within a foreign key( a
     * value of 1 represents the first column of the foreign key, a value of 2 would
     * represent the second column within the foreign key).
     * <LI><B>UPDATE_RULE</B> short {@code =>} What happens to a foreign key when
     ...
     * compatibility)
     * </UL>
     * <LI><B>DELETE_RULE</B> short {@code =>} What happens to the foreign key when
     * primary is deleted.
     * <UL>
     ...
     * has been deleted
     * </UL>
     * <LI><B>FK_NAME</B> String {@code =>} foreign key name (may be
     * <code>null</code>)
     * <LI><B>PK_NAME</B> String {@code =>} primary key name (may be
     * <code>null</code>)
    ...
    /**
     * For the column <code>UPDATE_RULE</code>, indicates that when the primary key
     * is updated, the foreign key (imported key) is changed to agree with it. For
     * the column <code>DELETE_RULE</code>, it indicates that when the primary key
     * is deleted, rows that imported that key are deleted.
     * <P>
     * A possible value for the columns <code>UPDATE_RULE</code> and
     * <code>DELETE_RULE</code> in the <code>ResultSet</code> objects returned by
     * the methods <code>getImportedKeys</code>, <code>getExportedKeys</code>, and
     * <code>getCrossReference</code>.
     */
    int importedKeyCascade = 0;

    /**
     * For the column <code>UPDATE_RULE</code>, indicates that a primary key may not
     * be updated if it has been imported by another table as a foreign key. For the
     * column <code>DELETE_RULE</code>, indicates that a primary key may not be
     * deleted if it has been imported by another table as a foreign key.
     * <P>
     * A possible value for the columns <code>UPDATE_RULE</code> and
     * <code>DELETE_RULE</code> in the <code>ResultSet</code> objects returned by
     * the methods <code>getImportedKeys</code>, <code>getExportedKeys</code>, and
    ...
    /**
     * For the columns <code>UPDATE_RULE</code> and <code>DELETE_RULE</code>,
     * indicates that when the primary key is updated or deleted, the foreign key
     * (imported key) is changed to <code>NULL</code>.
     * <P>
     * A possible value for the columns <code>UPDATE_RULE</code> and
     * <code>DELETE_RULE</code> in the <code>ResultSet</code> objects returned by
     * the methods <code>getImportedKeys</code>, <code>getExportedKeys</code>, and
    ...
    /**
     * For the columns <code>UPDATE_RULE</code> and <code>DELETE_RULE</code>,
     * indicates that if the primary key has been imported, it cannot be updated or
     * deleted.
     * <P>
     * A possible value for the columns <code>UPDATE_RULE</code> and
     * <code>DELETE_RULE</code> in the <code>ResultSet</code> objects returned by
    ...
    /**
     * For the columns <code>UPDATE_RULE</code> and <code>DELETE_RULE</code>,
     * indicates that if the primary key is updated or deleted, the foreign key
     * (imported key) is set to the default value.
     * <P>
     * A possible value for the columns <code>UPDATE_RULE</code> and
     * <code>DELETE_RULE</code> in the <code>ResultSet</code> objects returned by
     * the methods <code>getImportedKeys</code>, <code>getExportedKeys</code>, and
     ...
     * <P>
     * A possible value for the column <code>DEFERRABILITY</code> in the
     * <code>ResultSet</code> objects returned by the methods
     * <code>getImportedKeys</code>, <code>getExportedKeys</code>, and
     * <code>getCrossReference</code>.
     */
     ...
     * <OL>
     * <LI><B>PKTABLE_CAT</B> String {@code =>} primary key table catalog (may be
     * <code>null</code>)
     * <LI><B>PKTABLE_SCHEM</B> String {@code =>} primary key table schema (may be
     * <code>null</code>)
     ...
     * <LI><B>FKTABLE_SCHEM</B> String {@code =>} foreign key table schema (may be
     * <code>null</code>) being exported (may be <code>null</code>)
     * <LI><B>FKTABLE_NAME</B> String {@code =>} foreign key table name being
     * exported
     * <LI><B>FKCOLUMN_NAME</B> String {@code =>} foreign key column name being
     * exported
     * <LI><B>KEY_SEQ</B> short {@code =>} sequence number within foreign key( a
     * value of 1 represents the first column of the foreign key, a value of 2 would
     * represent the second column within the foreign key).
     * <LI><B>UPDATE_RULE</B> short {@code =>} What happens to foreign key when
     * primary is updated:
     * <UL>
     * <LI>importedNoAction - do not allow update of primary key if it has been
     * imported
     ...
     * Each foreign key column description has the following columns:
     * <OL>
     * <LI><B>PKTABLE_CAT</B> String {@code =>} parent key table catalog (may be
     * <code>null</code>)
     * <LI><B>PKTABLE_SCHEM</B> String {@code =>} parent key table schema (may be
     * <code>null</code>)
     ...
     * <UL>
     * <LI>importedNoAction - do not allow update of parent key if it has been
     * imported
     ...
     * <UL>
     * <LI>importedKeyNoAction - do not allow delete of parent key if it has been
     * imported
     * <LI>importedKeyCascade - delete rows that import a deleted key
     * <LI>importedKeySetNull - change imported key to <code>NULL</code> if its
     * primary key has been deleted
     * <LI>importedKeyRestrict - same as importedKeyNoAction (for ODBC 2.x
     * compatibility)
     * <LI>importedKeySetDefault - change imported key to default if its parent key
     * has been deleted
     * </UL>
     * <LI><B>FK_NAME</B> String {@code =>} foreign key name (may be
     * <code>null</code>)
     * <LI><B>PK_NAME</B> String {@code =>} parent key name (may be
     * <code>null</code>)
     * <LI><B>DEFERRABILITY</B> short {@code =>} can the evaluation of foreign key
     * constraints be deferred until commit
     * <UL>
     ...
     * </OL>
     *
     * @param parentCatalog  a catalog name; must match the catalog name as it is
     *                       stored in the database; "" retrieves those without a
     *                       catalog; <code>null</code> means drop catalog name from
     *                       the selection criteria
     * @param parentSchema   a schema name; must match the schema name as it is
     *                       stored in the database; "" retrieves those without a
     *                       schema; <code>null</code> means drop schema name from
     *                       the selection criteria
     ...
     * the database supports SQL structured types, then getTypeInfo() will return a
     ...
     * (may be <code>null</code>)
     * <LI><B>CREATE_PARAMS</B> String {@code =>} parameters used in creating the
     ...
     * <LI><B>SEARCHABLE</B> short {@code =>} can you use "WHERE" based on this
     * type:
     * <UL>
    ...
    /**
     * Indicates that it is not known whether a <code>NULL</code> value is allowed
     * for this data type.
     * <P>
     * A possible value for column <code>NULLABLE</code> in the
     * <code>ResultSet</code> object returned by the method
    ...
    /**
     * Indicates that <code>WHERE</code> search clauses are not supported for this
     * type.
     * <P>
    ...
    /**
     * Indicates that the data type can be only be used in <code>WHERE</code> search
     * clauses that use <code>LIKE</code> predicates.
     * <P>
     * A possible value for column <code>SEARCHABLE</code> in the
     * <code>ResultSet</code> object returned by the method
     * <code>getTypeInfo</code>.
    ...
    /**
     * Indicates that the data type can be only be used in <code>WHERE</code> search
     * clauses that do not use <code>LIKE</code> predicates.
     * <P>
     * A possible value for column <code>SEARCHABLE</code> in the
     * <code>ResultSet</code> object returned by the method
     * <code>getTypeInfo</code>.
    ...
    /**
     * Indicates that all <code>WHERE</code> search clauses can be based on this
     * type.
     * <P>
     * A possible value for column <code>SEARCHABLE</code> in the
     * <code>ResultSet</code> object returned by the method
     ...
     * <LI><B>INDEX_NAME</B> String {@code =>} index name; <code>null</code> when
     ...
     * </UL>
     * <LI><B>ORDINAL_POSITION</B> short {@code =>} column sequence number within
     * index; zero when TYPE is tableIndexStatistic
     * <LI><B>COLUMN_NAME</B> String {@code =>} column name; <code>null</code> when
     * TYPE is tableIndexStatistic
     * <LI><B>ASC_OR_DESC</B> String {@code =>} column sort sequence, "A" {@code =>}
     * ascending, "D" {@code =>} descending, may be <code>null</code> if sort
     * sequence is not supported; <code>null</code> when TYPE is tableIndexStatistic
     ...
     * <LI><B>PAGES</B> long {@code =>} When TYPE is tableIndexStatistic then this
     * is the number of pages used for the table, otherwise it is the number of
     * pages used for the current index.
     * <LI><B>FILTER_CONDITION</B> String {@code =>} Filter condition, if any. (may
     * be <code>null</code>)
     * </OL>
     *
     * @param catalog     a catalog name; must match the catalog name as it is
     ...
     * <P>
     * A possible value for column <code>TYPE</code> in the <code>ResultSet</code>
     * object returned by the method <code>getIndexInfo</code>.
     */
    ...
    /**
     * Indicates that this table index is not a clustered index, a hashed index, or
     * table statistics; it is something other than these.
     * <P>
     * A possible value for column <code>TYPE</code> in the <code>ResultSet</code>
     * object returned by the method <code>getIndexInfo</code>.
     */
     ...
     *
     * @param type defined in <code>sandbox.java.sql.ResultSet</code>
     * @return <code>true</code> if so; <code>false</code> otherwise
     * @exception SQLException if a database access error occurs
     * @see Connection
     * @since 1.2
     ...
     *
     * @param type        defined in <code>sandbox.java.sql.ResultSet</code>
     * @param concurrency type defined in <code>sandbox.java.sql.ResultSet</code>
     * @return <code>true</code> if so; <code>false</code> otherwise
     * @exception SQLException if a database access error occurs
     * @see Connection
     * @since 1.2
     ...
     * returned. They are ordered by <code>DATA_TYPE</code>, <code>TYPE_CAT</code>,
     * <code>TYPE_SCHEM</code> and <code>TYPE_NAME</code>. The type name parameter
     * may be a fully-qualified name. In this case, the catalog and schemaPattern
     * parameters are ignored.
     *
     * <P>
     * Each type description has the following columns:
     * <OL>
     * <LI><B>TYPE_CAT</B> String {@code =>} the type's catalog (may be
     * <code>null</code>)
     * <LI><B>TYPE_SCHEM</B> String {@code =>} type's schema (may be
     * <code>null</code>)
     * <LI><B>TYPE_NAME</B> String {@code =>} type name
     * <LI><B>CLASS_NAME</B> String {@code =>} Java class name
     * <LI><B>DATA_TYPE</B> int {@code =>} type value defined in
     * sandbox.java.sql.Types. One of JAVA_OBJECT, STRUCT, or DISTINCT
     * <LI><B>REMARKS</B> String {@code =>} explanatory comment on the type
     * <LI><B>BASE_TYPE</B> short {@code =>} type code of the source type of a
     * DISTINCT type or the type that implements the user-generated reference type
     * of the SELF_REFERENCING_COLUMN of a structured type as defined in
     * sandbox.java.sql.Types (<code>null</code> if DATA_TYPE is not DISTINCT or not
     ...
     *                        catalog; <code>null</code> means that the catalog name
     *                        should not be used to narrow the search
     * @param schemaPattern   a schema pattern name; must match the schema name as
     *                        it is stored in the database; "" retrieves those
     *                        without a schema; <code>null</code> means that the
     *                        schema name should not be used to narrow the search
     ...
     *                        DISTINCT) to include; <code>null</code> returns all
     *                        types
     ...
     *         <p>
     *         If <code>true</code> is returned, the JDBC driver must support the
     ...
     * <OL>
     * <LI><B>TYPE_CAT</B> String {@code =>} the UDT's catalog (may be
     * <code>null</code>)
     * <LI><B>TYPE_SCHEM</B> String {@code =>} UDT's schema (may be
     * <code>null</code>)
     ...
     * <code>null</code>)
     * <LI><B>TABLE_SCHEM</B> String {@code =>} type's schema (may be
     * <code>null</code>)
     ...
     * schema, type, and attribute name criteria. They are ordered by
     * <code>TYPE_CAT</code>, <code>TYPE_SCHEM</code>, <code>TYPE_NAME</code> and
     * <code>ORDINAL_POSITION</code>. This description does not contain inherited
     * attributes.
     * <P>
     ...
     * <LI><B>REMARKS</B> String {@code =>} comment describing column (may be
     * <code>null</code>)
     * <LI><B>ATTR_DEF</B> String {@code =>} default value (may be
     * <code>null</code>)
     * <LI><B>SQL_DATA_TYPE</B> int {@code =>} unused
     * <LI><B>SQL_DATETIME_SUB</B> int {@code =>} unused
     * <LI><B>CHAR_OCTET_LENGTH</B> int {@code =>} for char types the maximum number
     ...
     * <code>SQLException.getSQLState</code> is an X/Open (now know as Open Group)
     * SQL CLI SQLSTATE value.
     *
     * @since 1.4
     */
    int sqlStateXOpen = 1;
    ...
    /**
     * Indicates whether this data source supports the SQL {@code  ROWID} type, and
     * the lifetime for which a {@link RowId} object remains valid.
     *
     ...
     * <code>true</code> indicates that all open ResultSets are closed, even ones
     ...
     * property<br>
     * <li><b>DESCRIPTION</b> String{@code =>} A description of the property. This
     * will typically contain information as to where this property is stored in the
     * database.
     * </ol>
     * <p>
     ...
     * name that may be different then the <code>FUNCTION_NAME</code> for example
     ...
     * value is a NULL
     * <LI><B>ORDINAL_POSITION</B> int {@code =>} the ordinal position, starting
     * from 1, for the input and output parameters. A value of 0 is returned if this
     * row describes the function's return value. For result set columns, it is the
    ...
    /**
     * Indicates that it is not known whether the function returns a result or a
     * table.
     * <P>
     ...
     * Null is returned for data types where DECIMAL_DIGITS is not applicable.
     * <LI><B>NUM_PREC_RADIX</B> int {@code =>} Radix (typically either 10 or 2)
     * <LI><B>COLUMN_USAGE</B> String {@code =>} The allowed usage for the column.
     * The value returned will correspond to the enum name returned by
     * {@link PseudoColumnUsage#name PseudoColumnUsage.name()}
     * <LI><B>REMARKS</B> String {@code =>} comment describing column (may be
     * <code>null</code>)
     * <LI><B>CHAR_OCTET_LENGTH</B> int {@code =>} for char types the maximum number
     * of bytes in the column
     ...
     *
     * @return {@code true} if this database supports sharding; {@code false}
     *         otherwise
     * @exception SQLException if a database access error occurs
     * @since 9
     */
