 ...
 * as an SQL <code>DATE</code> value. A milliseconds value represents the number
 * of milliseconds that have passed since January 1, 1970 00:00:00.000 GMT.
 * <p>
 * To conform with the definition of SQL <code>DATE</code>, the millisecond
 * values wrapped by a <code>sandbox.java.sql.Date</code> instance must be
...
public class Date extends sandbox.java.util.Date {
     ...
     *
     * @param year  the year minus 1900; must be 0 to 8099. (Note that 8099 is 9999
     *              minus 1900.)
     * @param month 0 to 11
     * @param day   1 to 31
    ...
    /**
     * Constructs a <code>Date</code> object using the given milliseconds time
     * value. If the given milliseconds value contains time information, the driver
     * will set the time components to the time in the default time zone (the time
     * zone of the Java virtual machine running the application) that corresponds to
     * zero GMT.
     *
    ...
    /**
     * Sets an existing <code>Date</code> object using the given milliseconds time
     * value. If the given milliseconds value contains time information, the driver
     * will set the time components to the time in the default time zone (the time
     * zone of the Java virtual machine running the application) that corresponds to
     * zero GMT.
     ...
     *
     * @param s a <code>String</code> object representing a date in in the format
     *          "yyyy-[m]m-[d]d". The leading zero for <code>mm</code> and
     *          <code>dd</code> may also be omitted.
     * @return a <code>sandbox.java.sql.Date</code> object representing the given
     *         date
     * @throws IllegalArgumentException if the date given is not in the JDBC date
     *                                  escape format (yyyy-[m]m-[d]d)
     */
    public static Date valueOf(String s) {
        ...
        if ((firstDash > 0) && (secondDash > 0) && (secondDash < len - 1)) {
            if (firstDash == YEAR_LENGTH && (secondDash - firstDash > 1 && secondDash - firstDash <= MONTH_LENGTH + 1)
                ...
                String dy = s.substring(secondDash + 1, len);

                int year = Integer.parseInt(y, 10);
                int month = Integer.parseInt(m, 10);
                int day = Integer.parseInt(dy, 10);
     ...
     * Numbers will be zero-padded or truncated if the string representation of the
     * integer is smaller than or exceeds len, respectively.
     *
     * Should consider moving this to Integer and expose it through JavaLangAccess
     * similar to Integer::formatUnsignedInt
     * 
    ...
    /**
     * This method is deprecated and should not be used because SQL Date values do
    ...
    /**
     * Creates a {@code LocalDate} instance using the year, month and day from this
     * {@code Date} object.
     * 
     ...
     * used because SQL {@code Date} values do not have a time component.
     *
