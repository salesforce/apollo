 ...
 * built-in type that stores an XML value as a column value in a row of a
 * database table. By default drivers implement an SQLXML object as a logical
 * pointer to the XML data rather than the data itself. An SQLXML object is
 * valid for the duration of the transaction in which it was created.
 * <p>
 * The SQLXML interface provides methods for accessing the XML value as a
 * String, a Reader or Writer, or as a Stream. The XML value may also be
 * accessed through a Source or set as a Result, which are used with XML Parser
 * APIs such as DOM, SAX, and StAX, as well as with XSLT transforms and XPath
 * evaluations.
 * <p>
 * Methods in the interfaces ResultSet, CallableStatement, and
 * PreparedStatement, such as getSQLXML allow a programmer to access an XML
 * value. In addition, this interface has methods for updating an XML value.
 * <p>
 ...
 * performance without serializing to a stream representation and parsing the
 * XML.
 * <p>
 ...
 * </pre>
 * 
 * or, to perform XSLT transformations on the XML value using the XSLT in
 * xsltFile output to file resultFile:
 * 
 * <pre>
 ...
 * reading APIs are called: getBinaryStream(), getCharacterStream(),
 * getSource(), and getString(). Implementations may also change the state to
 * not writable when this occurs.
 * <p>
 * The state moves from writable to not writable once free() or any of the
 * writing APIs are called: setBinaryStream(), setCharacterStream(),
 * setResult(), and setString(). Implementations may also change the state to
 * not readable when this occurs.
 *
 * <p>
 * All methods on the <code>SQLXML</code> interface must be fully implemented if
...
public interface SQLXML {
    /**
     * This method closes this object and releases the resources that it held. The
     * SQL XML object becomes invalid and neither readable or writable when this
     * method is called.
     *
     ...
     * type sandbox.java.sql.Types of SQLXML.
     * <p>
     * The SQL XML object becomes not readable when this method is called and may
     * also become not writable depending on implementation.
     *
     ...
     * behavior of this method is the same as ResultSet.updateBinaryStream() when
     * the designated column of the ResultSet has a type sandbox.java.sql.Types of
     * SQLXML.
     * <p>
     * The SQL XML object becomes not writable when this method is called and may
     ...
     * present, the encoding of the stream is unicode. The behavior of this method
     * is the same as ResultSet.getCharacterStream() when the designated column of
     * the ResultSet has a type sandbox.java.sql.Types of SQLXML.
     * <p>
     * The SQL XML object becomes not readable when this method is called and may
     * also become not writable depending on implementation.
     *
     ...
     *                      getCause() method of the exception may provide a more
     *                      detailed exception, for example, if the stream does not
     *                      contain valid characters. An exception is thrown if the
     ...
     * 1.0 specification. Although an encoding declaration other than unicode may be
     * present, the encoding of the stream is unicode. The behavior of this method
     * is the same as ResultSet.updateCharacterStream() when the designated column
     * of the ResultSet has a type sandbox.java.sql.Types of SQLXML.
     * <p>
     * The SQL XML object becomes not writable when this method is called and may
     * also become not readable depending on implementation.
     *
     ...
     * according to section 2 and appendix B of the XML 1.0 specification. Although
     * an encoding declaration other than unicode may be present, the encoding of
     * the String is unicode. The behavior of this method is the same as
     * ResultSet.getString() when the designated column of the ResultSet has a type
     * sandbox.java.sql.Types of SQLXML.
     * <p>
     * The SQL XML object becomes not readable when this method is called and may
     * also become not writable depending on implementation.
     *
     ...
     * unicode code points for XML according to section 2 and appendix B of the XML
     * 1.0 specification. Although an encoding declaration other than unicode may be
     * present, the encoding of the String is unicode. The behavior of this method
     * is the same as ResultSet.updateString() when the designated column of the
     * ResultSet has a type sandbox.java.sql.Types of SQLXML.
     * <p>
     * The SQL XML object becomes not writable when this method is called and may
     * also become not readable depending on implementation.
     *
     ...
     * instance. Sources are used as inputs to XML parsers and XSLT transformers.
     * <p>
     ...
     *                    vendor specific Source implementation will be returned.
     ...
     *                      example, if an XML parser exception occurs. An exception
     ...
     * contents of the XML. Call the content handler with the contents of the XML
     * document to assign the values.
     * 
     * <pre>
     * SAXResult saxResult = sqlxml.setResult(SAXResult.class);
     ...
     *
     * @param <T>         the type of the class modeled by this Class object
     * @param resultClass The class of the result, or null. If resultClass is null,
     *                    a vendor specific Result implementation will be returned.
     *                    The following classes are supported at a minimum:
     * 
     *                    <pre>
