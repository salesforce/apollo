...
/**
 * <P>
 * A thin wrapper around the <code>java.util.Date</code> class that allows the
 * JDBC API to identify this as an SQL <code>TIME</code> value. The
 * <code>Time</code> class adds formatting and parsing operations to support the
 * JDBC escape syntax for time values.
 * <p>
...
public class Time extends java.util.Date {
    ...
    /**
     * Constructs a <code>Time</code> object initialized with the given values for
     * the hour, minute, and second. The driver sets the date components to January
     * 1, 1970. Any method that attempts to access the date components of a
     ...
     *
     * @deprecated Use the constructor that takes a milliseconds value in place of
     *             this constructor
     */
    ...
    public static Time valueOf(String s) {
        ...
        if (firstColon > 0 && secondColon > 0 && secondColon < len - 1) {
            String h = s.substring(0, firstColon);
            String m = s.substring(firstColon + 1, secondColon);
            String sec = s.substring(secondColon + 1, len);
    ...
    /**
     * Obtains an instance of {@code Time} from a {@link LocalTime} object with the
     * same hour, minute and second time value as the given {@code LocalTime}. The
     * nanosecond field from {@code LocalTime} is not part of the newly created
     * {@code Time} object.
     *
     ...
     * minute, and second time value as this {@code Time}. The nanosecond
     * {@code LocalTime} field will be set to zero.
     *
     ...
     * used because SQL {@code Time} values do not have a date component.
     *
