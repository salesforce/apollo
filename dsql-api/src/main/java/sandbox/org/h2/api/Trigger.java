...
public interface Trigger {
     ...
     * database is opened. The type of operation is a bit field with the appropriate
     * flags set. As an example, if the trigger is of type INSERT and UPDATE, then
     * the parameter type is set to (INSERT | UPDATE).
     *
     ...
     * transaction rollback will also rollback the operations that were done within
     * the trigger, if the operations occurred within the same database. If the
     * trigger changes state outside the database, a rollback trigger should be
     * used.
     * <p>
     * The row arrays contain all columns of the table, in the same order as defined
     * in the table.
     * </p>
     * <p>
    ...
    /**
     * This method is called when the database is closed. If the method throws an
     * exception, it will be logged, but closing the database will continue.
     */
