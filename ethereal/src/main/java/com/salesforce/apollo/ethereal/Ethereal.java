...
public class Ethereal {
    ...
    // A counterpart of process() that does not perform the setup phase.
    ...
    // The main processing entry point for Ethereal. Given two Config objects
    // (one for the setup phase and one for the main consensus), data source and
    // preblock sink, initialize two orderers and a channel between them used to
        ...
        Runnable start = () -> {
            config.executor().execute(() -> {
                try {
                    ...
                    logWTK(wtkey);
                    var orderer = new Orderer(Config.builderFrom(config).setWtk(wtkey).build(), null, makePreblock,
                                              Clock.systemUTC());
                    ord.set(orderer);
                    orderer.start(Coin.newFactory(config.pid(), wtkey), syn);
                } finally {
                    started.add(true);
                }
            });
