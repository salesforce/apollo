...
public class ByteArrayKeyAnalyzer extends KeyAnalyzer<byte[]> {
    ...
    public ByteArrayKeyAnalyzer(final int maxLengthInBits) {
        if (maxLengthInBits < 0) {
            throw new IllegalArgumentException("maxLengthInBits=" + maxLengthInBits);
        }
    ...
    public int bitIndex(final byte[] key, final int offsetInBits, final int lengthInBits, byte[] other,
                        final int otherOffsetInBits, final int otherLengthInBits) {
        if (other == null) {
            other = NULL;
        }
    ...
    public boolean isPrefix(final byte[] prefix, final int offsetInBits, final int lengthInBits, final byte[] key) {
        ...
        for (int i = 0; i < elements; i++) {
            if (isBitSet(prefix, i + offsetInBits, lengthInBits) != isBitSet(key, i, keyLength)) {
                return false;
            }
        }
        return true;
