...
public class MessageBuffer {
     ...
     */
    public Message put(long ts, byte[] bytes, Node from) {
        UUID id = GENERATOR.generate();
        Message update = createUpdate(id, ts, bytes, from.forSigning(), from.getId());
        put(update);
    ...
    public List<Message> updatesFor(List<MessageDigest> digests) {
        return digests.stream()
                      .map(digest -> state.get(uuid(digest.getId())))
                      .filter(m -> m != null)
                      .collect(Collectors.toList());
    }
    ...
    private Message createUpdate(UUID id, long ts, byte[] content, Signature signature, UUID from) {
        byte[] s;
        try {
        ...
        return new Message(new MessageDigest(uuidBits(from), uuidBits(id), 0, System.currentTimeMillis()),
