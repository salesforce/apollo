...
import com.salesforce.apollo.fireflies.View.ViewListener.Msg;
 ...
 * <a href= "https://munin.uit.no/bitstream/handle/10037/8712/article.pdf?sequence=7&isAllowed=y">Fireflies paper</a>.
 * 
...
public class View {
    ...
    public class Service {
        ...
        /**
         * @return The Duration to wait before a failed member is garbage collected from the view.
         */
        ...
        /**
         * Perform ye one ring round of gossip. Gossip is performed per ring, requiring 2 * tolerance + 1 gossip rounds
         * for a complete round, with t byzantine failures
         */
        public void gossip() {
            ...
            dispatcher.execute(() -> listeners.parallelStream().forEach(l -> {
                try {
                    l.round();
                } catch (Throwable e) {
        ...
        /**
         * Perform one round of monitoring the members assigned to this view. Monitor the unique set of members who are
         * the live successors of this member on the rings of this view.
         */
        public void monitor() {
            if (lastRing < 0) { return; }
         ...
         * 
         * @param ring
         *            - the index of the gossip ring the inbound member is gossiping from
         * @param digests
         *            - the inbound gossip
         * @param from
         *            - verified id of the sending member
         * @param certificate
         *            - the certificate for the sending member
         * @param note
         *            - the signed note for the sending member
        ...
        public Gossip rumors(int ring, Digests digests, UUID from, X509Certificate certificate, Signed note) {
            ...
            return successor == null || successor.equals(node) ? new Gossip(false,
                                                                            ...
                                                                            processCertificateDigests(from,
                                                                                                      digests.getCertificates()),
                                                                            processNoteDigests(from,
                                                                                               digests.getNotes()),
         ...
         * 
         * @param ring
         *            - the index of the gossip ring the gossip is originating from in this view
         * @param link
         *            - the outbound communications to the paired member
        ...
        /**
         * Drive one round of the View. This involves a round of gossip() and a round of monitor().
         */
        void oneRound() {
            comm.logDiag();
            try {
    ...
    public interface ViewListener {
        class Msg {
            ...
            public Msg(Member from, byte[] content) {
                super();
                this.from = from;
        ...
        /**
         * One gossip round has been completed
         */
        void round();
    }
    ...
    /**
     * The mapped set of open outbound connections with other members, mapped by member
     */
    ...
    /**
     * Lifecycle listeners for membership - randomly updated, observed by this process
     */
    private final Set<ViewListener> listeners = Collections.newSetFromMap(new ConcurrentHashMap<>());
    ...
    /**
     * Gossip rings - the list of rings that determine the magical low diameter connectivity
     */
    ...
    public void deregister(ViewListener listener) {
    ...
    public void publish(byte[] message) {
        messageBuffer.put(System.currentTimeMillis(), message, node);
    }

    public void register(ViewListener listener) {
        listeners.add(listener);
    }

    /**
     * Accuse the member on the list of rings. If the member has disabled the ring in its mask, do not issue the
     * accusation
     * 
     * @param member
     * @param ring
    ...
    /**
     * For bootstrap, add the seed as a fake, non crashed member. The note is signed by this node, and so will be
     * rejected by any other node as invalid. The gossip protocol of Fireflies provides redirects to what actual members
     * believe the view should be gossiping with, and so new members quickly converge on a common, valid view of the
     * membership
     * 
     * @param seed
     */
    ...
    /**
     * deserialize the DER encoded certificate. Validate it is signed by the pinned CA certificate trusted.
     * 
     * @param encoded
     * @return the deserialized certificate, or null if invalid or not correctly signed.
     */
    ...
    /**
     * @return the AccusationGossip for this view. This is the list of all accusation digests in the view. Do not add
     *         accusation digests for crashed members
     */
    ...
    /**
     * @return the CertificateGossip for this view. This is the list of all certicate digests of all members in the view
     */
    ...
    /**
     * @return the MessageGossip for this view. This is the list of all message digests of the view's messageBuffer
     */
    ...
    /**
     * @return the NoteGossip for this view. This is the list of the current Note digest for all members in the view
     */
    ...
    /**
     * @param ring
     *            - the ring to gossip on
     * @return the communication link for this ring, based on current membership state
     */
    ...
    /**
     * Check the link. If it stands accused, accuse this link on the supplied rings. In the fortunate, lucky case, the
     * list of rings is a singleton list - this is fundamental to a good quality mesh. But on the off chance there's an
     * overlap in the rings, we need to only monitor each link only once, or it skews the failure detection with the
     * short interval.
     * 
     * @param link
     *            - the ClientCommunications link to check
     * @param lastRing
     *            - the ring for this link
     */
    ...
    /**
     * The "second" message - response - in the anti-entropy protocol. Process the inbound certificates from the gossip.
     * Reconcile the differences between the view's state and the digests of the gossip. Update the reply with the list
     * of digests the view requires, as well as proposed updates based on the inbound digets that the view has more
    ...
    CertificateGossip processCertificateDigests(UUID from, List<CertificateDigest> certificates) {
        ...
        gossip.setDigests(certificates.stream().filter(cert -> {
            ...
            received.add(id);
            if (from.equals(id)) { return false; }
            Member member = view.get(id);
            if (member == null) { return true; }
    ...
    /**
     * Process the updates of the supplied juicy gossip. This is the Jesus Nut of state change driving the view.
     * 
     * @param gossip
     */
    ...
    void processUpdates(List<EncodedCertificate> certificatUpdates, List<Signed> noteUpdates,
        ...
        List<Msg> newMessages = messageBuffer.merge(messageUpdates, message -> validate(message))
                                             .stream()
                                             .map(m -> {
                                                 ...
                                                 if (from == null) {
                                                     ...
                                                     return new Msg(from, m.getContent().array());
                                                 }
                                             })
                                             .filter(m -> m != null)
                                             .collect(Collectors.toList());
     ...
     * @param ring
     * @param successor
     * @return the Gossip containing the successor's Certificate and Note from this view
     */
    ...
    void startRebutalTimer(Member m) {
        pendingRebutals.computeIfAbsent(m.getId(),
                                        id -> scheduler.schedule(() -> gc(m), service.getTimeoutInterval().toMillis(),
    ...
    /**
     * gather ye accusal gossip using the common digests and the list of updates based on the inboud requested digests
     * 
     * @param common
     * @param requested
     * @return
    ...
    /**
     * gather ye certificate gossip using the common digests and the list of updates based on the inboud requested
     * digests
     * 
     * @param common
     * @param requested
    ...
    /**
     * gather ye accusal note using the common digests and the list of updates based on the inboud requested digests
     * 
     * @param common
     * @param requested
     * @return
    ...
    NoteGossip updateNotes(List<NoteDigest> common, List<NoteDigest> requested) {
        return new NoteGossip(common, requested.stream()
                                               .map(digest -> uuid(digest.getId()))
                                               .map(id -> view.get(id))
                                               .filter(e -> e != null)
                                               .map(m -> m.getSignedNote())
     ...
     * 
     * @param message
     * @return true if the message is valid, false if from an unknown member or signature doesn't validate
     */
