 ...
 * @since 220
 */
public class FirefliesNettyCommunications extends CommonNettyCommunications
        implements FirefliesCommunications {
    volatile View view;
    ...
    public FfClientCommunications connectTo(Member to, Node from) {
        try {
            ...
            FfClientCommunications outbound = new FfClientCommunications(
                                                                         new NettyTlsTransceiver(to.getFirefliesEndpoint(),
                                                                                                 forClient(from).build(), eventGroup ) {
    ...
    protected MtlsServer newServer() {
        try {
            return new MtlsServer(view.getNode().getFirefliesEndpoint(),
                                  forServer(view.getNode()).clientAuth(ClientAuth.REQUIRE).build(),
                                  provider(),
                                  defaultBuiilder(), 1, view.getNode().getParameters().rings);
