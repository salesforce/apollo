...
/**
 * Represents a histogram of values. This class uses a {@link Segmenter} to determine which bucket to place a given
 * value into. Also stores the last MAX_HISTORY_SIZE entries which have been added to this histogram, in order. Note
 * that Histogram, by itself, is not synchronized.
 * 
 * @param <B>
 *            Bucket type. Often String, since buckets are typically used for their toString() representation.
 * @param <T>
 *            Type of value
 */
class Histogram<B, T> {
    ...
    public static class TreeMapSegmenter<T extends Comparable<T>> implements Segmenter<String, T> {
        ...
        public int segment(T value) {
            ...
            if (e == null) { throw new SegmenterException("Could not find bucket for: " + value); }
            return e.getValue();
        }
    ...
    /**
     * Interface to determine which bucket to place a value in. Segmenters should be immutable, so many histograms can
     * re-use the same segmenter.
     */
    interface Segmenter<B, T> {
         ...
         *             if value does not fit in a bucket.
         */
        int segment(T value);
    ...
    /**
     * Returns values recently added to this histogram. These are in reverse order (most recent first).
     */
