...
public class TestBLS192 { //
    ...
    @Test
    public void testBLS192() {
        RAND rng = new RAND();
        int BGS = BLS192.BGS;
        int BFS = BLS192.BFS;
        int G1S = BFS + 1; /* Group 1 Size compressed */
        int G2S = 4 * BFS + 1; /* Group 2 Size compressed */
        ...
        res = BLS192.KeyPairGenerate(IKM, S, W);
        if (res != 0)
        ...
        BLS192.core_sign(SIG, mess.getBytes(), S);
        System.out.print("Signature : 0x");
        printBinary(SIG);

        res = BLS192.core_verify(SIG, mess.getBytes(), W);
