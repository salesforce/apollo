...
public class Node {
    ...
    private final ControlledIdentifier identifier;
    private final Oracle               oracle;
    private final Parameters           params;
    private final Shard                shard;

    public Node(ControlledIdentifier id, Parameters.Builder params, KERL commonKERL, String dbURL,
                Path checkpointDir) throws SQLException {
        params = params.clone();
        var dir = checkpointDir.toFile();
        if (!dir.exists()) {
            if (!dir.mkdirs()) {
                throw new IllegalArgumentException("Cannot create directory: " + checkpointDir);
            }
        }
        if (!dir.isDirectory()) {
            throw new IllegalArgumentException("Must be a directory: " + checkpointDir);
        }
        this.identifier = id;
        this.commonKERL = commonKERL;
        SqlStateMachine sqlStateMachine = new SqlStateMachine(dbURL, new Properties(), dir);
