...
public class SubDomain extends Domain {
    ...
    private CommonCommunications<Delegation, DelegationService> comms;
    @SuppressWarnings("unused")
    private final MVMap<Ident, SignedDelegate>                  delegates;
    @SuppressWarnings("unused")
    private final MVMap<Ident, Ident>                           delegations;
    ...
    public SubDomain(ControlledIdentifierMember member, Builder params, Path checkpointBaseDir,
                     RuntimeParameters.Builder runtime, TransactionConfiguration txnConfig, Duration gossipInterval) {
        this(member, params, "jdbc:h2:mem:", checkpointBaseDir, runtime, txnConfig, gossipInterval);
    }

    public SubDomain(ControlledIdentifierMember member, Builder params, RuntimeParameters.Builder runtime,
                     TransactionConfiguration txnConfig, Duration gossipInterval) {
        this(member, params, tempDirOf(member.getIdentifier()), runtime, txnConfig, gossipInterval);
    }
    ...
    public SubDomain(ControlledIdentifierMember member, Builder prm, String dbURL, Path checkpointBaseDir,
        ...
        comms = params.communications()
                      .create(member, params.context().getId(), delegation(), "delegation",
                              r -> new DelegationServer(params.communications().getClientIdentityProvider(), r, null));
    ...
    public SubDomain(ControlledIdentifierMember member, Builder params, String dbURL, RuntimeParameters.Builder runtime,
                     TransactionConfiguration txnConfig, Duration gossipInterval) {
        this(member, params, dbURL, tempDirOf(member.getIdentifier()), runtime, txnConfig, gossipInterval);
    }
    ...
    private void handle(Optional<ListenableFuture<DelegationUpdate>> futureSailor,
        ...
        try {
            ...
            final var builder = DelegationUpdate.newBuilder().setRing(destination.ring()).setHave(have());
            update(builder);
            destination.link().update(builder.build());
        } finally {
            if (timer != null) {
                timer.stop();
            }
    ...
    private void update(DelegationUpdate.Builder builder) {
    }
}
