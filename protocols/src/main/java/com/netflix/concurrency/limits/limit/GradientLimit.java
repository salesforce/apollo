...
/**
 * Concurrency limit algorithm that adjust the limits based on the gradient of
 * change in the samples minimum RTT and absolute minimum RTT allowing for a
 * queue of square root of the current limit. Why square root? Because it's
 * better than a fixed queue size that becomes too small for large limits but
 * still prevents the limit from growing too much by slowing down growth as the
 * limit grows.
 */
public final class GradientLimit extends AbstractLimit {
    ...
    public int _update(final long startTime, final long rtt, final int inflight, final boolean didDrop) {
        ...
        // Reset or probe for a new noload RTT and a new estimatedLimit. It's necessary
        // to cut the limit
        // in half to avoid having the limit drift upwards when the RTT is probed during
        // heavy load.
        // To avoid decreasing the limit too much we don't allow it to go lower than the
        // queueSize.
        ...
        // allow it to be reduced by more than half to avoid aggressive load-sheding due
        // to
        // outliers.
        ...
        if ((int) newLimit != (int) estimatedLimit && LOG.isDebugEnabled()) {
            LOG.debug("New limit={} minRtt={} ms winRtt={} ms queueSize={} gradient={} resetCounter={}", (int) newLimit,
                      TimeUnit.NANOSECONDS.toMicros(rttNoLoad) / 1000.0, TimeUnit.NANOSECONDS.toMicros(rtt) / 1000.0,
                      queueSize, gradient, resetRttCounter);
        }
