 ...
 * To allow for better growth and stability at higher limits we set alpha=Max(3,
 * 10% of the current limit) and beta=Max(6, 20% of the current limit)
 */
public class VegasLimit extends AbstractLimit {
    ...
    private int updateEstimatedLimit(long rtt, int inflight, boolean didDrop) {
        ...
        if ((int) newLimit != (int) estimatedLimit && LOG.isDebugEnabled()) {
            LOG.debug("New limit={} minRtt={} ms winRtt={} ms queueSize={}", (int) newLimit,
                      TimeUnit.NANOSECONDS.toMicros(rtt_noload) / 1000.0, TimeUnit.NANOSECONDS.toMicros(rtt) / 1000.0,
                      queueSize);
        }
