...
/**
 * {@link Limiter} decorator that blocks the caller when the limit has been
 * reached. This strategy ensures the resource is properly protected but favors
 ...
 * 
 * Use this limiter only when the threading model allows the limiter to be
 * blocked.
 * 
 * @param <ContextT>
 */
public final class LifoBlockingLimiter<ContextT> implements Limiter<ContextT> {
    public static class Builder<ContextT> {
        ...
        /**
         * Set maximum timeout for threads blocked on the limiter. Default is 1 second.
         * 
         * @param timeout
         * @return Chainable builder
         */
        public Builder<ContextT> backlogTimeout(Duration timeout) {
            return backlogTimeoutMillis(timeout.toMillis());
        }

        /**
         * Function to derive the backlog timeout from the request context. This allows
         * timeouts to be set dynamically based on things like request deadlines.
         * 
         * @param mapper
         * @param units
         * @return
