...
public class Main {
	...
	public static LinkedHashMap<String, Command> listCommands() {
		List<Command> commands = Lists.newArrayList(new Init(), new Changelog(), new Status(), new Fork(), new Nuke(), new Drop(), new Query());
	...
	public static List<String> normalize(String[] args) {
		return Arrays.stream(args).map(String::trim).filter(input -> !Strings.isNullOrEmpty(input)).collect(Collectors.toList());
	}
}
