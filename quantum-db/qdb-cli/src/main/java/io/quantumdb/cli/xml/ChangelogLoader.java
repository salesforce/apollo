			...
			if (pointer != null) {
				for (int index = 1; index <= changeset.getOperations().size(); index++) {
					...
					if (!operation.equals(currentOperation)) {
						throw new IllegalStateException("Operation at index: " + index + " in changeset: " + changesetId + " differs from the one already defined in the database.");
					}
					if (!changesetId.equals(pointer.getChangeSet().getId())) {
						throw new IllegalStateException("The changeset with id: " + changesetId + " was " + "unexpected as the database already contains a different changeset in that position: " + pointer.getChangeSet().getId());
					}
				...
				List<Operation> operations = changeset.getOperations().stream().map(XmlOperation::toOperation).collect(Collectors.toList());
