...
public class VersionTraverser {
	...
	private static Optional<List<Version>> findPath(Version from, Version to, boolean traverseChildren, boolean traverseParent) {
	...
	public static List<Stage> verifyPathAndState(State state, Version from, Version to) {
		...
		List<Version> path = VersionTraverser.findChildPath(from, to).orElseThrow(() -> new IllegalStateException("No path from " + from.getId() + " to " + to.getId()));
