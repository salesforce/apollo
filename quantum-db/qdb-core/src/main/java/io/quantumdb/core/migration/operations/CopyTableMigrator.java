...
class CopyTableMigrator implements SchemaOperationMigrator<CopyTable> {
	...
	public void migrate(Catalog catalog, RefLog refLog, Version version, CopyTable operation) {
		...
		refLog.addTable(targetTableName, refId, version, sourceTableRef.getColumns().entrySet().stream().map(Entry::getValue).map(ColumnRef::ghost).collect(Collectors.toList()));
		...
		for (ForeignKey foreignKey : sourceTable.getForeignKeys()) {
			...
			targetTable.addForeignKey(foreignKey.getReferencingColumns()).named(foreignKey.getForeignKeyName()).onUpdate(foreignKey.getOnUpdate()).onDelete(foreignKey.getOnDelete()).referencing(referredTable, foreignKey.getReferredColumns());
		}
