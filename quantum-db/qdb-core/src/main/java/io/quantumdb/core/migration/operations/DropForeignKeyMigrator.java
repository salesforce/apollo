...
class DropForeignKeyMigrator implements SchemaOperationMigrator<DropForeignKey> {
	...
	public void migrate(Catalog catalog, RefLog refLog, Version version, DropForeignKey operation) {
		...
		ForeignKey matchedForeignKey = table.getForeignKeys().stream().filter(foreignKey -> foreignKey.getForeignKeyName().equals(operation.getForeignKeyName())).findFirst().orElseThrow(() -> new IllegalStateException("No foreign key exists in table: " + tableName + " with name: " + operation.getForeignKeyName()));
