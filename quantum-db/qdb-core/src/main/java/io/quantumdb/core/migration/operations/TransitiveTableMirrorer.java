...
class TransitiveTableMirrorer {
	static Set<String> mirror(Catalog catalog, RefLog refLog, Version version, String... tableNames) {
		...
		List<TableRef> tablesToMirror = Arrays.stream(tableNames).map(tableName -> refLog.getTableRef(parentVersion, tableName)).collect(Collectors.toList());
		while (!tablesToMirror.isEmpty()) {
			...
			tablesToMirror.addAll(referencingRefIds.stream().map(refId -> refLog.getTableRefById(refId)).collect(Collectors.toList()));
		}
