...
public class Catalog implements Copyable<Catalog> {
	...
	public Table getTable(String tableName) {
		checkArgument(!Strings.isNullOrEmpty(tableName), "You must specify a \'tableName\'");
		return tables.stream().filter(t -> t.getName().equals(tableName)).findFirst().orElseThrow(() -> new IllegalStateException("Catalog: " + name + " does not contain a table: " + tableName));
	}
	...
	public View getView(String viewName) {
		checkArgument(!Strings.isNullOrEmpty(viewName), "You must specify a \'viewName\'");
		return views.stream().filter(v -> v.getName().equals(viewName)).findFirst().orElseThrow(() -> new IllegalStateException("Catalog: " + name + " does not contain a view: " + viewName));
	}
	...
	public Sequence removeSequence(String sequenceName) {
		...
		tables.stream().flatMap(table -> table.getColumns().stream()).filter(column -> sequence.equals(column.getSequence())).forEach(Column::dropDefaultValue);
		return sequence;
	}

	private Sequence getSequence(String sequenceName) {
		checkArgument(!Strings.isNullOrEmpty(sequenceName), "You must specify a \'sequenceName\'");
		return sequences.stream().filter(sequence -> sequence.getName().equals(sequenceName)).findFirst().orElseThrow(() -> new IllegalStateException("Catalog: " + name + " does not contain a sequence: " + sequenceName));
	}
	...
	public ImmutableSet<ForeignKey> getForeignKeys() {
		return ImmutableSet.copyOf(tables.stream().flatMap(table -> table.getForeignKeys().stream()).collect(Collectors.toSet()));
	}

	public ImmutableSet<Index> getIndexes() {
		return ImmutableSet.copyOf(tables.stream().flatMap(table -> table.getIndexes().stream()).collect(Collectors.toSet()));
	}
	...
	public Catalog copy() {
		...
		for (ForeignKey foreignKey : getForeignKeys()) {
			...
			source.addForeignKey(foreignKey.getReferencingColumns()).named(foreignKey.getForeignKeyName()).onDelete(foreignKey.getOnDelete()).onUpdate(foreignKey.getOnUpdate()).referencing(target, foreignKey.getReferredColumns());
		}
	...
	public Set<String> getTablesReferencingTable(String tableName) {
		return tables.stream().filter(table -> table.referencesTable(tableName)).map(Table::getName).collect(Collectors.toSet());
	}
	...
	public boolean equals(final java.lang.Object o) {
		...
		if (this$sequences == null ? other$sequences != null : !this$sequences.equals(other$sequences)) return false;
		return true;
	}
