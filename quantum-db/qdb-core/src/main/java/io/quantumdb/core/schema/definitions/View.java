...
public class View implements Copyable<View>, Comparable<View> {
	...
	void setParent(Catalog parent) {
		if (parent == null && this.parent != null) {
			checkState(!this.parent.containsView(name), "The view: " + name + " is still present in the catalog: " + this.parent.getName() + ".");
		} else if (parent != null && this.parent == null) {
			checkState(!parent.containsTable(name), "The catalog: " + parent.getName() + " already contains a different table with the name: " + name);
			checkState(!parent.containsView(name), "The catalog: " + parent.getName() + " already contains a different view with the name: " + name);
		}
		this.parent = parent;
	}
	...
	public View rename(String newName) {
		...
		if (parent != null) {
			checkState(!parent.containsTable(newName), "Catalog: " + parent.getName() + " already contains view with name: " + newName);
		}
		this.name = newName;
		return this;
	}
