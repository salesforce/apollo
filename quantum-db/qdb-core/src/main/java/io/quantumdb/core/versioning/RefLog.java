...
public class RefLog {
	...
	public static abstract class DataRef {
		...
		public boolean equals(Object other) {
			if (other instanceof DataRef) {
				...
				return new EqualsBuilder().append(name, otherRef.getName()).append(refId, otherRef.getRefId()).append(versions, otherRef.getVersions()).isEquals();
			}
			return false;
		}
		...
		public java.lang.String toString() {
			return "RefLog.DataRef(name=" + this.getName() + ", refId=" + this.getRefId() + ", versions=" + this.getVersions() + ")";
		}
	}
	...
	public static class ViewRef extends DataRef {
		...
		public boolean equals(final java.lang.Object o) {
			...
			if (!(o instanceof RefLog.ViewRef)) return false;
	...
	public static class TableRef extends DataRef {
		...
		public Set<TableRef> getBasedOn() {
			return columns.values().stream().flatMap(column -> column.getBasedOn().stream()).map(ColumnRef::getTable).distinct().collect(Collectors.toSet());
		}

		public Set<TableRef> getBasisFor() {
			return columns.values().stream().flatMap(column -> column.getBasisFor().stream()).map(ColumnRef::getTable).distinct().collect(Collectors.toSet());
		}

		public TableRef ghost(String newRefId, Version version) {
			Collection<ColumnRef> newColumns = columns.values().stream().map(ColumnRef::ghost).collect(Collectors.toList());
			markAsAbsent(version);
		...
		public boolean equals(Object other) {
			if (other instanceof TableRef) {
				...
				return super.equals(other) && new EqualsBuilder().append(columns, otherRef.getColumns()).append(inboundSyncs, otherRef.getInboundSyncs()).append(outboundSyncs, otherRef.getOutboundSyncs()).isEquals();
			}
			return false;
		}
	...
	public static class SyncRef {
		...
		private SyncRef(String name, String functionName, Map<ColumnRef, ColumnRef> columnMapping) {
			...
			List<TableRef> sources = columnMapping.keySet().stream().map(ColumnRef::getTable).distinct().collect(Collectors.toList());
			List<TableRef> targets = columnMapping.values().stream().map(ColumnRef::getTable).distinct().collect(Collectors.toList());
		...
		public boolean equals(Object other) {
			if (other instanceof SyncRef) {
				...
				EqualsBuilder builder = new EqualsBuilder().append(name, otherRef.getName()).append(functionName, otherRef.getFunctionName());
				if (source != null) {
		...
		@java.lang.SuppressWarnings("all")
		public java.lang.String toString() {
			return "RefLog.SyncRef(name=" + this.getName() + ", functionName=" + this.getFunctionName() + ", columnMapping=" + this.getColumnMapping() + ", source=" + this.getSource() + ", target=" + this.getTarget() + ")";
		}
	}
	...
	public RefLog bootstrap(Catalog catalog, Version version) {
		...
		catalog.getTables().forEach(table -> {
			TableRef tableRef = addTable(table.getName(), table.getName(), version, table.getColumns().stream().map(column -> new ColumnRef(column.getName())).collect(Collectors.toList()));
			log.debug("Added TableRef: {} (id: {}) for version: {} with columns: {}", table.getName(), table.getName(), version, tableRef.getColumns().keySet());
		});
	...
	public RefLog fork(Version version) {
		...
		checkArgument(refMapping.isEmpty() || refMapping.keySet().contains(parent), "You cannot fork to a version whose parent is not in the RefLog!");
	...
	public Collection<TableRef> getTableRefs() {
		return ImmutableSet.copyOf(refMapping.values().stream().filter(ref -> ref instanceof TableRef).map(ref -> (TableRef) ref).collect(Collectors.toSet()));
	}
	...
	public Collection<TableRef> getTableRefs(Version version) {
		checkArgument(version != null, "You must specify a version!");
		return ImmutableSet.copyOf(refMapping.get(version).stream().filter(ref -> ref instanceof TableRef).map(ref -> (TableRef) ref).collect(Collectors.toSet()));
	}
	...
	public TableRef getTableRef(Version version, String tableName) {
		...
		return refMapping.get(version).stream().filter(ref -> ref instanceof TableRef).map(ref -> (TableRef) ref).filter(table -> table.getName().equals(tableName)).findFirst().orElseThrow(() -> new IllegalArgumentException("Version: " + version.getId() + " does not contain a TableRef with tableName: " + tableName));
	}
	...
	public TableRef getTableRefById(String refId) {
		...
		return refMapping.values().stream().filter(ref -> ref instanceof TableRef).map(ref -> (TableRef) ref).filter(table -> table.getRefId().equals(refId)).findFirst().orElseThrow(() -> new IllegalArgumentException("No table with id: " + refId));
	}
	...
	public TableRef replaceTable(Version version, String sourceTableName, String targetTableName, String refId) {
		...
		return new TableRef(this, targetTableName, refId, version, tableRef.getColumns().values().stream().map(columnRef -> new ColumnRef(columnRef.getName(), Lists.newArrayList(columnRef))).collect(Collectors.toList()));
	}
	...
	public void dropTable(TableRef tableRef) {
		...
		List<Version> versions = refMapping.entries().stream().filter(entry -> entry.getValue().equals(tableRef)).map(Entry::getKey).collect(Collectors.toList());
	...
	public TableRef addTable(String name, String refId, Version version, Collection<ColumnRef> columns) {
		...
		if (matches > 0) {
			throw new IllegalStateException("A TableRef for tableName: " + name + " is already present for version: " + version.getId());
		}
	...
	public Collection<ViewRef> getViewRefs() {
		return ImmutableSet.copyOf(refMapping.values().stream().filter(ref -> ref instanceof ViewRef).map(ref -> (ViewRef) ref).collect(Collectors.toSet()));
	}
	...
	public Collection<ViewRef> getViewRefs(Version version) {
		checkArgument(version != null, "You must specify a version!");
		return ImmutableSet.copyOf(refMapping.get(version).stream().filter(ref -> ref instanceof ViewRef).map(ref -> (ViewRef) ref).collect(Collectors.toSet()));
	}
	...
	public ViewRef getViewRef(Version version, String viewName) {
		...
		return refMapping.get(version).stream().filter(ref -> ref instanceof ViewRef).map(ref -> (ViewRef) ref).filter(view -> view.getName().equals(viewName)).findFirst().orElseThrow(() -> new IllegalArgumentException("Version: " + version.getId() + " does not contain a ViewRef with viewName: " + viewName));
	}
	...
	public ViewRef getViewRefById(String refId) {
		...
		return refMapping.values().stream().filter(ref -> ref instanceof ViewRef).map(ref -> (ViewRef) ref).filter(view -> view.getRefId().equals(refId)).findFirst().orElseThrow(() -> new IllegalArgumentException("No view with id: " + refId));
	}
	...
	public void dropView(ViewRef viewRef) {
		...
		List<Version> versions = refMapping.entries().stream().filter(entry -> entry.getValue().equals(viewRef)).map(Entry::getKey).collect(Collectors.toList());
	...
	public ViewRef addView(String name, String refId, Version version) {
		...
		if (matches > 0) {
			throw new IllegalStateException("A ViewRef for viewName: " + name + " is already present for version: " + version.getId());
		}
	...
	public boolean equals(final java.lang.Object o) {
		if (o == this) return true;
		if (!(o instanceof RefLog)) return false;
		...
		if (this$refMapping == null ? other$refMapping != null : !this$refMapping.equals(other$refMapping)) return false;
		...
		if (this$activeVersions == null ? other$activeVersions != null : !this$activeVersions.equals(other$activeVersions)) return false;
		return true;
	}
