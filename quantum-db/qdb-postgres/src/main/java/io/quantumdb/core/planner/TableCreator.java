...
public class TableCreator {
	...
	private void createTable(Connection connection, Table table) throws SQLException {
		...
		List<String> identityColumns = table.getIdentityColumns().stream().map(Column::getName).collect(Collectors.toList());
		...
		if (shouldOwnSequence) {
			for (Map.Entry<String, String> sequence : sequences.entrySet()) {
				execute(connection, new QueryBuilder().append("ALTER SEQUENCE " + sequence.getKey()).append("OWNED BY " + table.getName() + "." + sequence.getValue()));
			}
		}
	}
