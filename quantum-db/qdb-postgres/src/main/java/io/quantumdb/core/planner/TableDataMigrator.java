...
class TableDataMigrator {
	...
	void migrateData(NullRecords nullRecords, Table source, Table target, Version from, Version to, Set<String> migratedColumns, Set<String> columnsToMigrate) throws SQLException, InterruptedException {
		...
		MigratorFunction initialMigrator = SelectiveMigratorFunction.createMigrator(nullRecords, refLog, source, target, from, to, BATCH_SIZE, Stage.INITIAL, migratedColumns, columnsToMigrate);
		MigratorFunction successiveMigrator = SelectiveMigratorFunction.createMigrator(nullRecords, refLog, source, target, from, to, BATCH_SIZE, Stage.CONSECUTIVE, migratedColumns, columnsToMigrate);
		...
		try (Connection connection = backend.connect()) {
			...
			while (true) {
				...
				if (lastProcessedId.isEmpty()) {
					...
					List<String> values = successiveMigrator.getParameters().stream().map(parameterName -> asExpression(lastProcessedId.get(stripEscaping(parameterName)))).collect(Collectors.toList());
					migrator.append("SELECT * FROM " + successiveMigrator.getName() + "(").append(Joiner.on(", ").join(values) + ");");
				}
				...
				log.info("Migration data source: {} target: {}, now at identity: {}, took: {} ms", source.getName(), target.getName(), lastProcessedId, innerEnd - innerStart);
	...
	private Map<String, Object> queryHighestId(Table from) throws SQLException {
		List<String> identityColumns = from.getIdentityColumns().stream().map(Column::getName).collect(Collectors.toList());
		try (Connection connection = backend.connect()) {
			try (Statement statement = connection.createStatement()) {
				String query = new QueryBuilder().append("SELECT " + Joiner.on(", ").join(identityColumns)).append("FROM " + from.getName()).append("ORDER BY " + Joiner.on(" DESC, ").join(identityColumns) + " DESC").append("LIMIT 1").toString();
