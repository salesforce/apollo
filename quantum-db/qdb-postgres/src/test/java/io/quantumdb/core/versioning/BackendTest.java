...
public class BackendTest {
    ...
    public void testPersistingAndLoadingSimpleTestCase() throws SQLException {
        ...
        Catalog catalog = new Catalog(
                "public").addSequence(sequence)
                         .addTable(new Table("table_1")
                                                       .addColumn(new Column("id", bigint(), sequence, AUTO_INCREMENT,
                                                               IDENTITY))
        ...
        Changelog changelog = new Changelog(RandomHasher.generateHash(),
                new ChangeSet("init", "QuantumDB", "Initial import")).addChangeSet("add_table", "Michael de Jong",
                                                                                   addColumn("table", "admin", bool(),
                                                                                             "false", NOT_NULL));

        RefLog refLog = new RefLog();
        TableRef table1 = refLog.addTable("table", "table_1", changelog.getRoot(), new ColumnRef("id"),
                                          new ColumnRef("name"));

        TableRef table2 = refLog.addTable("table", "table_2", changelog.getLastAdded(),
                                          new ColumnRef("id", table1.getColumn("id")),
                                          new ColumnRef("name", table1.getColumn("name")), new ColumnRef("admin"));

        refLog.addSync("trigger_1", "sync_1",
                       ImmutableMap.<ColumnRef, ColumnRef>builder()
                                   .put(table1.getColumn("id"), table2.getColumn("id"))
                                   .put(table1.getColumn("name"), table2.getColumn("name"))
                                   .build());

        refLog.addSync("trigger_2", "sync_2",
                       ImmutableMap.<ColumnRef, ColumnRef>builder()
