...
final class JdbcUtils {
    ...
    static class DialectPool {
        ...
        private final LoadingCache<Pair<SqlDialectFactory, DataSource>, SqlDialect> cache = CacheBuilder.newBuilder()
                                                                                                        .softValues()
        ...
        public SqlDialect get(SqlDialectFactory dialectFactory, DataSource dataSource) {
            final Pair<SqlDialectFactory, DataSource> key = Pair.of(dialectFactory, dataSource);
            return cache.getUnchecked(key);
        }
    }

    /**
     * Builder that calls {@link ResultSet#getObject(int)} for every column, or
     * {@code getXxx} if the result type is a primitive {@code xxx}, and returns an
    ...
    static class ObjectArrayRowBuilder implements Function0<Object[]> {
        ...
        public static Function1<ResultSet, Function0<Object[]>> factory(final List<Pair<ColumnMetaData.Rep, Integer>> list) {
            return resultSet -> {
                try {
                    return new ObjectArrayRowBuilder(resultSet,
     ...
     * see a new data source, we have to open a connection to find out what database
     * product and version it is.
     */
    static class DataSourcePool {
        public static final DataSourcePool INSTANCE = new DataSourcePool();

        private final LoadingCache<List<String>, BasicDataSource> cache = CacheBuilder.newBuilder()
                                                                                      .softValues()
                                                                                      .build(CacheLoader.from(DataSourcePool::dataSource));

        private static BasicDataSource dataSource(List<String> key) {
