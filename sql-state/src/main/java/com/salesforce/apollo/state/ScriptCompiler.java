...
public class ScriptCompiler {
     ...
     * @return <code>true</code> if {@link #getCompiledScript(String)} can be
     *         called.
     */
     ...
     */
    public CompiledScript getCompiledScript(String source) throws ScriptException {
        final String lang;
        ...
        CompiledScript compiledScript = jsEngine.compile(source);
     ...
     * "com.sun.tools.javac.Main" if available. If not, it tries to run "javac" in a
     * separate process.
     *
    ...
    private static void javacProcess(File javaFile) {
        exec("javac", "-sourcepath", COMPILE_DIR, "-d", COMPILE_DIR, "-encoding", "UTF-8", javaFile.getAbsolutePath());
    }
    ...
    private static synchronized void javacSun(File javaFile) {
        ...
        try {
            ...
            // Bugfix: Here we should check exit status value instead of parsing javac
            // output text.
            // Because of the output text is different in different locale environment.
            // @since 2018-07-20 little-pan
            final Integer status = (Integer) compile.invoke(javac, (Object) new String[] { "-sourcepath", COMPILE_DIR,
                                                                                           // "-Xlint:unchecked",
                                                                                           "-d", COMPILE_DIR,
                                                                                           "-encoding", "UTF-8",
    ...
    private static final class GroovyCompiler {
        ...
        static {
            ...
            try {
                ...
                String[] importsArray = { "java.sql.Connection", "java.sql.Types", "java.sql.ResultSet",
        ...
        public static Class<?> parseClass(String source, String packageAndClassName) {
            ...
            try {
                Object codeSource = Utils.newInstance("groovy.lang.GroovyCodeSource", source,
                                                      packageAndClassName + ".groovy", "UTF-8");
