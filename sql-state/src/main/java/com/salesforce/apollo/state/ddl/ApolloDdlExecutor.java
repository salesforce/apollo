...
/**
 * @author hal.hildebrand Executes DDL commands.
 *         <p>
 *         Given a DDL command that is a sub-class of {@link SqlNode},
 *         dispatches the command to an appropriate {@code execute} method. For
 *         example, "CREATE TABLE" ({@link SqlCreateTable}) is dispatched to
 *         {@link #execute(SqlCreateTable, CalcitePrepare.Context)}.
 */
public class ApolloDdlExecutor extends DdlExecutorImpl {
    public ApolloDdlExecutor() {
    }
    ...
    static SqlNode renameColumns(SqlNodeList columnList, SqlNode query) {
        ...
        final SqlCall from = SqlStdOperatorTable.AS.createCall(p,
                                                               ImmutableList.<SqlNode>builder()
                                                                            ...
                                                                            .addAll(columnList)
                                                                            .build());
        return new SqlSelect(p, null, selectList, from, null, null, null, null, null, null, null, null);
    }
    ...
    public void execute(SqlCreateForeignSchema create, CalcitePrepare.Context context) {
        ...
        if (subSchema0 != null) {
            if (!create.getReplace() && !create.ifNotExists) {
                throw SqlUtil.newContextException(create.name.getParserPosition(), RESOURCE.schemaExists(pair.right));
            }
        }
     ...
     * {@code DROP MATERIALIZED VIEW}, {@code DROP TYPE}, {@code DROP VIEW}
     * commands.
     */
    ...
    public void execute(SqlCreateMaterializedView create, CalcitePrepare.Context context) {
        ...
        final ViewTableMacro viewTableMacro = ViewTable.viewMacro(pair.left.plus(), sql, schemaPath,
        ...
        table.key = MaterializationService.instance()
                                          .defineMaterialization(pair.left, null, sql, schemaPath, pair.right, true,
                                                                 true);
    }

    /** Executes a {@code DROP MATERIALIZED VIEW} command. */
    public void execute(SqlDropMaterializedView drop, CalcitePrepare.Context context) {
    ...
    public void execute(SqlCreateTable create, CalcitePrepare.Context context) {
        ...
        if (create.query != null) {
            ...
            final ViewTableMacro viewTableMacro = ViewTable.viewMacro(pair.left.plus(), sql, pair.left.path(null),
                                                                      context.getObjectPath(), false);
            final TranslatableTable x = viewTableMacro.apply(ImmutableList.of());
        ...
        for (Ord<SqlNode> c : Ord.zip(columnList)) {
            if (c.e instanceof SqlColumnDeclaration) {
                ...
                final ColumnStrategy strategy = f.getType().isNullable() ? ColumnStrategy.NULLABLE
        ...
        // Table does not exist. Create it.
        pair.left.add(pair.right, new MutableArrayTable(pair.right, RelDataTypeImpl.proto(storedRowType),
    ...
    public void execute(SqlCreateView create, CalcitePrepare.Context context) {
        ...
        final SqlNode q = renameColumns(create.columnList, create.query);
        final String sql = q.toSqlString(CalciteSqlDialect.DEFAULT).getSql();
        final ViewTableMacro viewTableMacro = ViewTable.viewMacro(schemaPlus, sql, pair.left.path(null),
                                                                  context.getObjectPath(), false);
        final TranslatableTable x = viewTableMacro.apply(ImmutableList.of());
        Util.discard(x);
