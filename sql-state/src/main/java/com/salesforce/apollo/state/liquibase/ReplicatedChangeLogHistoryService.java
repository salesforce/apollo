...
public class ReplicatedChangeLogHistoryService extends StandardChangeLogHistoryService {
    ...
    public void tag(final String tagString) throws DatabaseException {
        ...
        int totalRows = Scope.getCurrentScope()
                             .getSingleton(ExecutorService.class)
                             .getExecutor("jdbc", database)
                             .queryForInt(new SelectFromDatabaseChangeLogStatement(new ColumnConfig().setName("COUNT(*)",
                                                                                                              true)));
        if (totalRows == 0) {
            final var now = DateTimeUtils.now();
            String dateString = String.format("%s.%s", now.getEpochSecond(), now.getNano());
            ChangeSet emptyChangeSet = new ChangeSet(dateString, "liquibase", false, false, "liquibase-internal", null,
                                                     null, getDatabase().getObjectQuotingStrategy(), null);
