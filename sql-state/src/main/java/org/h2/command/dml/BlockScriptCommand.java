 ...
 * SCRIPT
 */
public class BlockScriptCommand extends ScriptBase {
    ...
    public BlockScriptCommand(Session session) {
        super(session);
    }
    ...
    private int generateInsertValues(int count, Table table) throws IOException {
        ...
        int heightIndex = table.getColumn(Parser.BLOCK_HEIGHT).getColumnId();
        ...
        while (cursor.next()) {
            Row row = cursor.get();
            if (row.getValue(heightIndex).getLong() > maxBlockHeight) {
                continue; // Do not include row
            }
            if (builder == null) {
