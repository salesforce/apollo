...
public class UpdateTimestampIntegrationTest extends IntegrationBase {
    ...
    public void testRewriting() {
        CalciteAssert.model(TargetDatabase.makeJournalledModel(versionType))
                     .query("UPDATE \"" + virtualSchemaName
                             + "\".\"depts\" SET \"department_name\"='First' WHERE \"deptno\" = 3")
    ...
    public void testRewritingWithIncorrectVersionType() {
        try {
            CalciteAssert.model(TargetDatabase.makeJournalledModel(versionType).replace("TIMESTAMP", "BIGINT"))
                         .query("UPDATE \"" + virtualSchemaName
                                 + "\".\"depts\" SET \"department_name\"='First' WHERE \"deptno\" = 3")
                         .withHook(Hook.PROGRAM, JournalledJdbcRuleManager.program())
                         .explainContains("PLAN=JdbcToEnumerableConverter\n" + "  JdbcTableModify(table=[["
    ...
    public void testNonJournalled() {
        CalciteAssert.model(TargetDatabase.makeJournalledModel(versionType))
                     .query("UPDATE \"" + virtualSchemaName
                             + "\".\"depts_journal\" SET \"department_name\"='First' WHERE \"deptno\" = 3")
                     .withHook(Hook.PROGRAM, JournalledJdbcRuleManager.program())
                     .explainContains("PLAN=JdbcToEnumerableConverter\n" + "  JdbcTableModify(table=[["
                             + virtualSchemaName
                             + ", depts_journal]], operation=[UPDATE], updateColumnList=[[department_name]], sourceExpressionList=[['First']], flattened=[false])\n"
                             + "    JdbcProject(deptno=[$0], department_name=[$1], version_number=[$2], subsequent_version_number=[$3], EXPR$0=['First'])\n"
                             + "      JdbcFilter(condition=[=($0, 3)])\n" + "        JdbcTableScan(table=[["
                             + virtualSchemaName + ", depts_journal]])\n")
                     .planUpdateHasSql("UPDATE \"" + actualSchemaName
                             + "\".\"depts_journal\" SET \"department_name\" = 'First'\nWHERE \"deptno\" = 3", 1);
    }
}
