 ...
 *
 */
public class BinderClient implements BinderService, Link {
    ...
    @Override
    public CompletableFuture<Boolean> publish(KERL kerl) {
    ...
    public CompletableFuture<Boolean> append(KeyEvent keyEvent) {
        ...
        CompletableFuture<Boolean> f = new CompletableFuture<>();
        ListenableFuture<Empty> result = client.append(request);
        result.addListener(() -> {
            try {
                result.get();
            } catch (InterruptedException e) {
