...
public class AltingConnectionServerImpl<T> extends AltingConnectionServer<T> {
    ...
    private AltingChannelInput<ConnectionClientMessage<T>> openIn;

    private AltingChannelInput<ConnectionClientMessage<T>> furtherRequestIn;

    private ChannelInput<ConnectionClientMessage<T>> currentInputChannel;

    private ChannelOutput<ConnectionServerMessage<T>> toClient = null;
     ...
     */
    protected AltingConnectionServerImpl(AltingChannelInput<ConnectionClientMessage<T>> openIn,
                                         AltingChannelInput<ConnectionClientMessage<T>> furtherRequestIn) {
        super(openIn);
        this.openIn = openIn;
    ...
    public T request() throws IllegalStateException {
        ...
        ConnectionClientMessage<T> msg = currentInputChannel.read();

        if (currentServerState == SERVER_STATE_CLOSED) {
            if (msg instanceof ConnectionClientOpenMessage<T> m) {
                // channel to use to reply to client
                toClient = m.replyChannel;
