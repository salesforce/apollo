...
/**
 * This defines an interface for an <i>any-to-one</i> integer channel, safe for
 ...
 * {@link Channel#any2oneInt() <i>synchronising</i>},
 * {@link Channel#any2oneInt(jcsp.util.ints.ChannelDataStoreInt)
 * <i>buffered</i>}, {@link Channel#any2oneInt(int) <i>poisonable</i>} or
 * {@link Channel#any2oneInt(jcsp.util.ints.ChannelDataStoreInt,int)
 * <i>both</i>} <i>(i.e. buffered and poisonable)</i>.
 * </P>
 * <H2>Description</H2> <TT>Any2OneChannelInt</TT> is an interface for an
 * integer channel which is safe for use by many writing processes but only one
 * reader. Writing processes compete with each other to use the channel. Only
 * the reader and one writer will actually be using the channel at any one time.
 * This is managed by the channel &ndash; user processes just read from or write
 * to it.
 * </P>
 * <P>
