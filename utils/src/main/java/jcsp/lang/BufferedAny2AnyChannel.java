 ...
 * <H2>Description</H2> <TT>BufferedAny2AnyChannel</TT> implements an any-to-any
 * object channel with user-definable buffering. It is safe for use by any
 * number of reading or writing processes. Reading processes compete with each
 * other to use the channel. Writing processes compete with each other to use
 * the channel. Only the reader and one writer will actually be using the
 * channel at any one time. This is taken care of by
 * <TT>BufferedAny2AnyChannel</TT> -- user processes just read from or write to
 * it.
 * <P>
