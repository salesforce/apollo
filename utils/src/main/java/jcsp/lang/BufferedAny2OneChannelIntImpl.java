 ...
 * <H2>Description</H2> <TT>BufferedAny2OneChannelIntImpl</TT> implements an
 * any-to-one integer channel with user-definable buffering. It is safe for use
 * by many writing processes but only one reader. Writing processes compete with
 * each other to use the channel. Only the reader and one writer will actually
 * be using the channel at any one time. This is taken care of by
 * <TT>BufferedAny2OneChannelIntImpl</TT> -- user processes just read from or
 * write to it.
 * <P>
