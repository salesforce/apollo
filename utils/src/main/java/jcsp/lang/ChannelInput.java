 ...
 * {@link ChannelOutput#write <code>write</code>} method (invoked by the writing
 * process) also completes. When a {@link #startRead <code>startRead</code>}
 * completes, the matching {@link ChannelOutput#write <code>write</code>} method
 * does not complete until the reader process invokes an {@link #endRead
 * <code>endRead</code>}. Actions performed by the reader in between a
 * {@link #startRead <code>startRead</code>} and {@link #endRead
 * <code>endRead</code>} make up an <i>extended rendezvous</i>.
 * 
 * <P>
 * <TT>ChannelInput</TT> variables are used to hold channels that are going to
 * be used only for <I>input</I> by the declaring process. This is a security
 * matter -- by declaring a <TT>ChannelInput</TT> interface, any attempt to
 * <I>output</I> to the channel will generate a compile-time error. For example,
 * the following code fragment will not compile:
 *
 * <PRE>
 ...
 * </PRE>
 *
 * When configuring a <TT>CSProcess</TT> with input channels, they should be
 * declared as <TT>ChannelInput</TT> (or, if we wish to be able to make choices
 * between events, as <TT>AltingChannelInput</TT>) variables. The actual channel
 * passed, of course, may belong to <I>any</I> channel class that implements
 * <TT>ChannelInput</TT> (or <TT>AltingChannelInput</TT>).
 * <P>
 * The <TT>Object</TT> returned can be cast into the actual class the reader
 * process expects. If the reader can handle more than one class of
 * <TT>Object</TT> (similar to tagged protocols in <I><B>occam</B></I>), checks
 * should be made before casting.
 * <H2>Examples</H2>
 * <H3>Discard data</H3>
 * 
 * <PRE>
...
public interface ChannelInput<T> extends Poisonable {
     ...
     * {@link ChannelOutput#write <code>write</code>} method). The writer is unaware
     * of the extended nature of the communication.
     * </p>
     * <p>
