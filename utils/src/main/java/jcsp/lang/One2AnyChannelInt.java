...
/**
 * This defines the interface for a <i>one-to-any</i> integer channel, safe for
 ...
 * {@link Channel#one2anyInt() <i>synchronising</i>},
 * {@link Channel#one2anyInt(jcsp.util.ints.ChannelDataStoreInt)
 * <i>buffered</i>}, {@link Channel#one2anyInt(int) <i>poisonable</i>} or
 * {@link Channel#one2anyInt(jcsp.util.ints.ChannelDataStoreInt,int)
 * <i>both</i>} <i>(i.e. buffered and poisonable)</i>.
 * </P>
 * <H2>Description</H2> <TT>One2AnyChannelInt</TT> is an interface for a channel
 * which is safe for use by many reading processes but only one writer. Reading
 * processes compete with each other to use the channel. Only one reader and the
 ...
 * Currently, broadcasting has to be managed by writing an active process (see
